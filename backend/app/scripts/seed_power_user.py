...
async def seed_chats(user_id: str):
    ...
    for topic in chat_topics:
        ...
        convo = ConversationModel(
            conversation_id=convo_id, description=topic, is_system_generated=False
        )
        ...
        messages = [
            ...
            MessageModel(
                type="ai",
                response=f"Sure, I can help with {topic}. What's on your mind?",
            ),
        ...
        await update_messages(
            UpdateMessagesRequest(conversation_id=convo_id, messages=messages), USER
        )
    print("Chats seeded.")
...
async def seed_todos(user_id: str):
    ...
    for config in projects_config:
        proj = await ProjectService.create_project(
            ProjectCreate(name=config["name"], color=config["color"]), user_id
        )
