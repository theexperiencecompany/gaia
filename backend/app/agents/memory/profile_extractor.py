...
async def extract_user_profiles_with_llm(
    emails: List[Dict], user_name: str | None = None
) -> Dict[str, str]:
    """
    Extract user profile URLs from emails using LLM.

    For each platform, filters last 10 emails from that platform,
    uses LLM to extract username, validates it, and builds profile URL.

    Args:
        emails: All user emails
        user_name: User's full name to help identify their username
    ...
    """
    start_time = time.time()
    logger.info(f"Starting LLM-based profile extraction from {len(emails)} emails")

    # Group emails by platform
    platform_emails = {}
    for platform in PLATFORM_CONFIG.keys():
        filtered = filter_emails_by_platform(emails, platform)
        if filtered:
            platform_emails[platform] = filtered
            logger.info(f"Found {len(filtered)} emails from {platform}")

    if not platform_emails:
        logger.info("No platform emails found")
        return {}

    # Extract usernames in parallel using LLM
    extraction_tasks = []
    for platform, platform_email_list in platform_emails.items():
        task = extract_username_with_llm(platform, platform_email_list, user_name)
        extraction_tasks.append((platform, task))

    # Wait for all extractions
    results = await asyncio.gather(*[task for _, task in extraction_tasks])

    # Validate and build URLs
    user_profiles = {}
    for (platform, _), username in zip(extraction_tasks, results):
        if validate_username(username, platform):
            url = build_profile_url(username, platform)
            user_profiles[platform] = url
            logger.info(f"✓ Valid profile extracted: {url}")
        else:
            logger.warning(f"✗ Invalid/not found username for {platform}: {username}")

    elapsed = time.time() - start_time
    logger.info(
        f"LLM profile extraction completed in {elapsed:.2f}s: "
        f"found {len(user_profiles)}/{len(platform_emails)} valid profiles"
    )
