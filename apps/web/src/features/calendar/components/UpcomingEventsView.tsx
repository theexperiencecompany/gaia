...
function isEventPassed(event: GoogleCalendarEvent): boolean {
  const now = new Date();
                ...
                {events.map((event) => {
                  ...
                  return (
                    <button
                      type="button"
                      key={event.id}
                      className="relative flex w-full cursor-pointer items-start gap-2 rounded-lg p-2 pl-5 text-left transition-colors hover:bg-zinc-700/30 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
