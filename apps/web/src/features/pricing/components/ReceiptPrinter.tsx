...
function ReceiptPrinterMachine({
  ...
  return (
    <div className={cn(machineClassName, className)} {...props}>
      ...
      <div
        aria-hidden="true"
        className="absolute inset-x-6 bottom-[var(--printer-inset)] z-40 h-2 rounded-[0.25rem] border border-zinc-950 bg-zinc-950 shadow-inner shadow-zinc-950 dark:border-zinc-50 dark:bg-zinc-50 dark:shadow-zinc-50"
      />
    </div>
  );
}
...
function ReceiptPrinterScreen({
  ...
  return (
    <div
      className={cn(
        "relative z-10 isolate overflow-hidden rounded-[var(--printer-inner-radius)] border border-zinc-950 bg-zinc-950 p-4 text-zinc-50 shadow-inner shadow-zinc-950/80 after:pointer-events-none after:absolute after:inset-0 after:z-20 after:rounded-[inherit] after:shadow-[inset_0_0_24px_4px_color-mix(in_oklab,#09090b_52%,transparent)] after:content-[''] dark:border-zinc-50 dark:bg-zinc-100 dark:text-zinc-950 dark:shadow-zinc-50/80 dark:after:shadow-[inset_0_0_24px_4px_color-mix(in_oklab,#fafafa_52%,transparent)]",
        className,
      )}
      {...props}
    >
    ...
    >
      <AnimatePresence initial={false} mode="sync">
        {isComplete ? (
          ...
          <m.span
            animate={{ opacity: 1, transform: "scale(1)" }}
            className="col-start-1 row-start-1 grid place-items-center text-zinc-500 dark:text-zinc-600"
            exit={{
              opacity: animate ? 0 : 1,
              transform: move ? "scale(0.96)" : "scale(1)",
            }}
      ...
      >
        <AnimatePresence initial={false} mode="sync">
          <m.div
            ...
            className="col-start-1 row-start-1 truncate font-medium text-xs leading-none text-zinc-400 dark:text-zinc-600"
            exit={{
              opacity: animate ? 0 : 1,
