...
export function AgencyWorkflowsDemo() {
  ...
  useEffect(() => {
    if (!inView) return;
    WORKFLOW_STEPS.slice(1).forEach((_, i) => {
      timersRef.current.push(
        setTimeout(() => setCurrentStep(i + 2), (i + 1) * 500),
      );
    });
    timersRef.current.push(
      setTimeout(() => setDone(true), WORKFLOW_STEPS.length * 500 + 300),
    );
    ...
    >
      <div className="min-h-0 flex-1 overflow-y-auto no-scrollbar">
        <div className="mb-5 rounded-2xl bg-zinc-800 p-4">
          ...
          <span className="text-xs text-zinc-500">Every Friday at 4:00 PM</span>
        </div>
