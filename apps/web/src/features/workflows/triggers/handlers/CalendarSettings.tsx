/**
 * Google Calendar Trigger Settings
 *
 * UI configuration for calendar triggers:
...
export function CalendarSettings({
  ...
  const selectedCalendars = useMemo(() => {
    ...
    if (!calIds || !Array.isArray(calIds)) return new Set(["primary"]);
    ...
    if (allIds.length > 0 && allIds.every((id) => mappedIds.has(id))) {
      mappedIds.add("all");
    }

    return mappedIds;
  }, [calendars, triggerData?.calendar_ids]);
