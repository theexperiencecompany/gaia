...
Styling: the dark-card pattern in `DESIGN.md` (outer `rounded-3xl bg-zinc-800 p-4`, inner `rounded-2xl bg-zinc-900 p-3`, no borders) is the **default**, not a hard rule. A card with a deliberately distinct presentation may diverge — e.g. `SendNotificationSection.tsx` mimics an OS notification banner. `RateLimitCard.tsx` is a good reference for the default look.
