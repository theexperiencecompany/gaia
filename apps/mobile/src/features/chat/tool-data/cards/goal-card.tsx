...
function computeProgress(goal: GoalItem): number {
  ...
  if (roadmapTasks.length > 0) {
    const completedCount = roadmapTasks.filter(
      (n) => n.data.isComplete,
    ).length;
...
function GoalItemCard({ goal }: { goal: GoalItem }) {
  ...
  return (
    <View className="rounded-xl bg-zinc-900 p-3 mb-2">
      <Text className="text-sm font-semibold text-zinc-100 mb-1" numberOfLines={2}>
        {goal.title}
      </Text>
      ...
      <View className="mb-2">
        <View className="flex-row items-center justify-between mb-1">
          ...
          <Text className={`text-xs font-semibold ${getProgressColor(progress)}`}>
            {progress}%
          </Text>
        </View>
