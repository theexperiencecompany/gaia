...
const ToolRow: React.FC<ToolRowProps> = ({ tool, index }) => {
  ...
  const successProgress = spring({
    frame: frame - tool.completeAt,
    fps,
    config: { damping: 12 },
  });
  const successScale = isDone
    ? interpolate(successProgress, [0, 0.5, 1], [0, 1.3, 1.0])
    : 0;
    ...
    >
      ...
      <div
        style={{
          flex: 1,
          paddingTop: 4,
          paddingBottom: index < TOOLS.length - 1 ? 16 : 0,
        }}
      >
        <div
          style={{
...
export const S07_ChatToolCalls: React.FC = () => {
  ...
  const exitY = interpolate(exitP, [0, 1], [0, -30], {
    ...
    >
      ...
      {/* {TOOLS.map((_, i) => (
        <Sequence key={i} from={i * 8}>
          <Audio src={SFX.uiSwitch} volume={0.12} />
        </Sequence>
      ))} */}
      {/* Accordion expand */}
      <Sequence from={20}>
      ...
      {/* Completion ticks */}
      {TOOLS.map((tool) => (
        <Sequence key={tool.completeAt} from={tool.completeAt}>
      ...
      >
        ...
        >
          ...
          >
            ...
            <div>
              {TOOLS.map((tool, i) => (
                <ToolRow key={i} tool={tool} index={i} />
              ))}
            </div>
          </div>
        </div>
