...
<Note>
  ...
  Loading](/developers/extra/lazy-loading).
</Note>
...
<Steps>
  <Step title="Copy environment files">
    ```bash cp apps/api/.env.example apps/api/.env cp apps/web/.env.example
    apps/web/.env ```
  </Step>
  <Step title="Set required variables">
    At minimum, set these in `apps/api/.env`: ```bash ENV=development
    MONGO_DB=mongodb://mongo:27017/gaia REDIS_URL=redis://redis:6379
    WORKOS_API_KEY=your-workos-api-key WORKOS_CLIENT_ID=your-workos-client-id
    WORKOS_COOKIE_PASSWORD=your-32-char-random-secret ```
  </Step>
  <Step title="Start development">```bash mise dev ```</Step>
</Steps>
