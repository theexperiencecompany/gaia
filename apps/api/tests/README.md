...
- **`integration/real/`** — Real-database tests. Production functions run unmodified against real Redis/Postgres/MongoDB/ChromaDB (see `integration/real/README.md`), not mocks. The memory-engine suite lives in `integration/real/memory/`. Run via `nx run api:test:service`.
