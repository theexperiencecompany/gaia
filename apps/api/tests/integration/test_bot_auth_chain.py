...
TEST_BOT_API_KEY = "test-bot-api-key-for-integration-tests"  # pragma: allowlist secret
...
class TestBotTokenService:
    ...
    def test_missing_secret_raises_value_error(self) -> None:
        ...
        with (
            patch.object(settings, "BOT_SESSION_TOKEN_SECRET", None),
            pytest.raises(ValueError, match="BOT_SESSION_TOKEN_SECRET is required"),
        ):
            create_bot_session_token(
                user_id=TEST_USER_ID,
                platform=TEST_PLATFORM,
