...
| `test_subscription_event_race.py` | `apply_subscription_event` (`app/services/payments/subscription_events.py`), the one writer of subscription state | Two lifecycle deliveries for one subscription read the same row: the write is conditional on the `last_event_at` it was decided from, so the older event can never overwrite the newer status — and the newer one still applies when it writes second |
