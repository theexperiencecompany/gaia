...
def _served_by(upstream: str) -> AIMessage:
    """Build a reply carrying the upstream name the provider-name patch restores."""
...
async def test_the_row_holds_no_prompt_or_completion_text() -> None:
    """Asserted on the row that actually gets written, not just on the model: the ledger must never become a copy of the conversation."""
    doc = await _record()
...
async def test_a_table_priced_call_says_so() -> None:
    """cost_source is what makes provider-price coverage measurable; a table guess recorded as a provider price would overstate coverage."""
...
async def test_a_ledger_failure_never_fails_the_metered_call() -> None:
    """The one sanctioned silent degrade in this path. The money is already booked in Redis + usage_daily and the call is already on the wide event, so a Mongo blip must cost a row of analytics, not the user's reply."""
    with (
        patch("app.services.llm_metering.record_model_call_usage", new_callable=AsyncMock),
        patch.object(
...
def test_the_aggregators_own_name_is_not_recorded_as_the_upstream(reported: str | None) -> None:
    """OpenRouter routes one model id across upstreams whose rates differ by more than 10x, so provider exists to tell them apart. Recording the aggregator would make every row claim the same provider and answer the question wrong."""
...
def test_the_upstream_the_patch_restored_is_recorded_verbatim(upstream: str) -> None:
    """openrouter_provider_name_patch puts the real serving upstream on the reply under PROVIDER_NAME_METADATA_KEY; ChatOpenRouter itself drops it. Reading that key is what turns provider from a column that was always null into the one that makes the >10x per-upstream rate spread queryable."""
...
def test_a_reply_the_patch_never_stamped_records_no_upstream() -> None:
    """Non-OpenRouter lanes (direct Gemini, the sim lane) never carry the key. Absent is recorded as absent, never guessed from the model id."""
...
async def test_the_ledger_row_records_the_upstream_that_served_the_call() -> None:
    """End to end through the metering seam: the name the patch stamped is what lands in the row, so group by provider over the ledger answers which upstream the money actually went to."""
    doc = await _record(
...
# workflow_execution_id, job_id and the task name exist only on the wide event's
# ContextVar boundary (ARQ's arq_task, the workflow task), not config.configurable.
...
async def test_a_workflow_boundary_with_no_execution_id_yet_records_none() -> None:
    """A workflow event is stamped in stages; a call metered before the execution id lands must record its absence, not an empty-string id."""
    log.reset()
...
async def test_a_call_outside_any_boundary_invents_no_worker_identity() -> None:
    """An HTTP request path has no ARQ job and no workflow run. The fields are absent, never a fabricated id."""
    log.reset()
    try:
        doc = await _record()
    finally:
...
async def test_a_non_mapping_workflow_field_does_not_crash_the_metered_call() -> None:
    """log.set takes arbitrary values, and some call sites stamp workflow=<id string>. Reaching into that for execution_id would raise inside the metering path of a call that already succeeded."""
    log.reset()
...
async def test_the_ledger_write_is_detached_from_the_users_turn() -> None:
    """The insert is spawned, not awaited: nothing downstream depends on it, and holding a turn open for a Mongo round-trip to write an analytics row would be paying user-visible latency for observability. The task is named so a stuck one is identifiable in a task dump rather than anonymous."""
    with (
        patch("app.services.llm_metering.record_model_call_usage", new_callable=AsyncMock),
...
async def test_the_rows_timestamp_is_timezone_aware_utc() -> None:
    """created_at is the TTL key. A naive datetime is interpreted as UTC by Mongo but compares wrong against every tz-aware value in the codebase, so a ledger query by time would silently skew by the server's offset."""
    doc = await _record()
...
async def test_a_child_lane_with_no_conversation_id_recovers_it_from_the_thread() -> None:
    """An executor run whose config carries only the wrapped thread still has to join its parent turn in the ledger. Without the fallback its calls land with no conversation and the turn's cost breakdown loses them."""
    doc = await _record(
        context=LLMCallContext(
            agent_name="executor_agent",
            background=False,
...
def test_the_surface_the_turn_came_from_is_recorded_verbatim(source: str) -> None:
    """conversation_source is set by the entry point — the chat endpoint's X-Client-Type header or the bot endpoint's platform — and inherited by every child agent, so an executor call reports its root turn's surface."""
    assert resolve_channel({"conversation_source": source}) == source


def test_a_workflow_run_is_its_own_channel() -> None:
    """Background runs have nobody to attribute a surface to, so they are told apart by what they do carry. A workflow is the one background lane whose cost anyone asks about by name."""
...
def test_background_work_with_no_surface_is_system() -> None:
    """The documented rule. Live, 11 background rows (memory:*, chatbot) came out null instead — their configurable carries neither a source nor a source_category, so the rule never applied and COGS-by-channel could not account for them at all."""
...
def test_a_call_inside_an_executor_run_keeps_the_turns_surface() -> None:
    """Live defect: a comms/follow-up call made INSIDE an executor run was labelled system though the turn came from web. The executor opens its own wide-event boundary, and an auxiliary call there gets a bare config — so the surface has to come from the boundary the run stamped, or COGS-by-channel under-counts web on exactly the expensive turns."""
    log.reset()
...
def test_the_runs_own_configurable_still_wins_over_the_boundary() -> None:
    """The boundary is a fallback, not an override: a nested call that DOES know its own surface must not be relabelled with the enclosing run's."""
    log.reset()
    log.set(conversation_source="web")
    try:
...
def test_a_bag_with_no_surface_at_all_records_none() -> None:
    """Auxiliary one-shots built from a bare {"user_id": ...} config have no originating surface. None is the honest answer, not a default."""
...
def test_an_explicit_surface_wins_over_the_background_derivation() -> None:
    """A workflow that was kicked off from a real surface keeps that surface — the derivation is a fallback for runs nobody started interactively."""
...
def test_the_finish_reason_is_read_from_the_streamed_reply() -> None:
    """ChatOpenRouter merges generation_info into response_metadata on the streaming path, which is every graph call."""
...
def test_the_native_finish_reason_is_the_fallback() -> None:
    """The non-streaming path leaves finish_reason in generation_info, which never reaches an AIMessage — only the upstream's own value is copied on."""
...
# Classified by exception TYPE, never message text: provider messages embed model
# ids, request ids and prompt fragments that change without notice.
...
def test_two_failures_of_one_family_with_different_messages_group_together() -> None:
    """The property that makes the field usable: message text varies per call, the bucket must not."""
...
async def test_a_failed_call_books_no_money_and_no_tokens() -> None:
    """The attempts did burn tokens upstream, but nothing reported them. A guessed number would sit in the same column real spend is summed from."""
    with (
...
def test_a_provider_throttling_us_is_its_own_family() -> None:
    """The one failure that is our fault to fix (back off, spread load) rather than the upstream's — it must not be lumped in with the outages."""
    assert classify_error_family(_RATE_LIMITED) == "rate_limit"


def test_a_rejected_request_is_not_an_outage() -> None:
    """A 400 means we sent something wrong; nothing upstream is down. Counting it as unavailability sends an incident response after a bug."""
...
def test_an_upstream_timeout_is_a_timeout_not_an_outage() -> None:
    """Both are unavailability in the broad sense, and the specific answer is the useful one — which is why the checks are ordered."""
...
async def test_the_error_row_carries_the_calls_identity_like_any_other() -> None:
    """An error row that could not be filtered by conversation, lane or surface would be a count with nothing to drill into."""
    with (
        patch("app.services.llm_metering.record_model_call_usage", new_callable=AsyncMock),
        patch.object(llm_metering.llm_calls_repository, "create", new_callable=AsyncMock) as create,
    ):
...
async def test_a_failed_call_is_never_charged_to_the_user() -> None:
    """Whatever the context said, a call that produced nothing cannot count against an allowance."""
    with (
        patch("app.services.llm_metering.record_model_call_usage", new_callable=AsyncMock),
        patch.object(llm_metering.llm_calls_repository, "create", new_callable=AsyncMock) as create,
    ):
...
async def test_a_ledger_failure_on_an_error_row_still_does_not_raise() -> None:
    """The degrade rule applies to error rows too — an outage must not be made worse by the code that records it."""
    with (
        patch.object(
            llm_metering.llm_calls_repository,
            "create",
...
async def test_the_error_rows_insert_is_named_for_what_it_is() -> None:
    """Named distinctly from the success insert so a stuck or failing error-row task is identifiable in a task dump rather than anonymous."""
...
# Failed calls emit an llm_call wide event too: live evidence showed 4 failed
# POSTs produced 0 log lines and 2 ledger rows, leaving failures recoverable
# nowhere else.
...
async def test_the_failure_event_books_no_spend() -> None:
    """It has to be summable alongside the success events without inflating anything — a failed call cost us nothing we can account for."""
    with (
        patch.object(llm_metering.llm_calls_repository, "create", new_callable=AsyncMock),
        patch.object(llm_metering.log, "info") as emitted,
    ):
