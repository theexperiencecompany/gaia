...
async def test_what_a_done_part_found_is_carried_into_the_next_part() -> None:
    ...
    assert "FOUND SO FAR, what this run has chosen and read: Part 1: Gemini TTS, 217 points" in (
        _goal(gateway)
    )


async def test_what_a_part_not_yet_done_found_is_its_progress_never_a_finished_parts_finding() -> (
    None
):
        ...
        return schema.model_validate(
            {
                "requirements": ["it"],
                "done": True,
                "evidence": [{"requirement": "it", "kind": "fact", "source": url}],
                "findings": "",
            }
        )
