...
from app.services.browser.jev.policy import JevDecision
...
async def test_a_failing_writer_is_explained_with_its_error_cut_to_200_characters(
    ...
    model, _, _, _ = _model(flights_state, [("NAVIGATE", None)], [RuntimeError("x" * 300)])
...
async def test_a_decision_with_nothing_to_act_on_idles_the_step_naming_the_operation(
    flights_state, monkeypatch
) -> None:
    ...
    assert (model._history[-1].kind, model._history[-1].text) == (
        "error",
        "Jev chose SELECT without a usable target.",
    )
