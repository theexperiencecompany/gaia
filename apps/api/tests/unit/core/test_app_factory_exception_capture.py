...
def _cors_only_middleware(app: FastAPI) -> None:
    """Install only CORS middleware, skipping Redis/WorkOS.

    create_app() raw installs Redis-backed middleware, so on a runner without
    Redis a ConnectionError surfaces before the route is reached.
    """
...
def _app_with_boom_route(user: dict[str, Any] | None) -> FastAPI:
    ...
    resets request.state.user = None at the top of its dispatch (auth.py:148), so
    anything seeded outside it is wiped before the route runs.
    """
    app = _hermetic_app()
...
async def test_unauthenticated_crash_is_still_captured_without_an_id(
    ...
    """An anonymous crash must still reach error tracking — dropping it would hide every failure on the public surface."""
    posthog_client = MagicMock()
    posthog_provider(available=True, client=posthog_client)
