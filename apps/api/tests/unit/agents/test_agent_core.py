...
def _fresh_config() -> dict:
    """A config object the calling test owns.

    ``_core_agent_logic`` MUTATES ``config["configurable"]``, so the shared
    module-level FAKE_CONFIG cannot be handed out directly — one test would see
...
def _common_patches():
    ...
    return {
        ...
        "build_config": patch(
            ...
            new_callable=AsyncMock,
            side_effect=lambda *args, **kwargs: _fresh_config(),
        ),
...
class TestCoreAgentLogic:
    ...
    async def test_returns_graph_state_config(self):
        ...
        with (
            ...
            patches["log"],
            # Pinned, like every other class here that asserts on the config: a
            # developer .env sets ENV=development, and the run then stamps the
            # dev executor pick onto the configurable it returns.
...
class TestTheLaneTheRunResolves:
    ...
    async def test_every_field_the_run_carries_reaches_build_agent_config(self):
        ...
        with (
            ...
            patches["log"],
            # Same pin as TestTheDevModelSelector: unpinned, this asserts
            # dev_option is None while a developer .env (ENV=development,
            # DEV_DEFAULT_MODEL set) resolves a real one.
            patch.object(agent_module.settings, "ENV", "production"),
        ):
            await _core_agent_logic(
                request=request,
