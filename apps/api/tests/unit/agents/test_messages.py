...
class TestMemoryRecallCap:
    """The volatile memory-recall slot is capped head+tail so its per-turn
    churn stays within a bounded cache footprint (the provider's prompt cache
    is small; every changed byte writes new blocks that evict the conversation
    chain)."""

    @pytest.mark.asyncio
    async def test_recall_content_capped_head_and_tail(self) -> None:
        ...
        big_core = "core memory line. " * 600  # ~9.6k chars — over the cap
        with (
            patch(
                "app.helpers.message_helpers._get_core_memory_section",
                new=AsyncMock(return_value=big_core),
            ),
            patch(
            ...
            result = await build_dynamic_context_messages(user_id="u1", query="q", user_name="Dev")
        assert result.memory_recall is not None
        content = result.memory_recall.content
        assert len(content) <= MEMORY_RECALL_MAX_CHARS + 64
        # Head (core memory) and tail (directives) survive; the middle is cut.
        assert content.startswith("core memory line. ")
        assert len(content) > MEMORY_RECALL_HEAD_CHARS
        assert "recall truncated to keep the prompt cache warm" in content

    @pytest.mark.asyncio
    async def test_small_recall_untouched(self) -> None:
        from app.helpers.message_helpers import build_dynamic_context_messages

        small_core = "a small core memory. " * 10
        with (
            patch(
                "app.helpers.message_helpers._get_core_memory_section",
                new=AsyncMock(return_value=small_core),
            ),
            patch(
                "app.helpers.message_helpers._get_user_memories_section",
                new=AsyncMock(return_value=""),
            ...
            result = await build_dynamic_context_messages(user_id="u1", query="q", user_name="Dev")
        assert result.memory_recall is not None
        assert result.memory_recall.content == small_core
