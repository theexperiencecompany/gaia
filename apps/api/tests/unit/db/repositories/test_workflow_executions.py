...
def _raw(execution_id: str, started_at: str) -> dict[str, object]:
    """Return a stored execution as the driver hands it back, before _to_model."""
    return {
...
class TestFindRecentWithTrace:
    async def test_it_reads_finished_runs_success_or_failed_that_recorded_a_trace(
        ...
        """A failed fire that ran steps carries its trace; filtering it out showed the next run the fire before and repeated it."""
    ...
    async def test_it_asks_the_driver_for_the_newest_runs_and_only_as_many_as_asked(
        ...
        """Recent is entirely the sort direction plus the cap: an ascending sort or an unbounded read both defeat it."""
    ...
    async def test_it_returns_the_rows_in_the_order_the_driver_yielded(
        ...
        """The sort already put the newest first, so the finder must hand the rows back as they came."""
