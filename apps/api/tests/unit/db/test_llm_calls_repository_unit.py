...
async def test_each_row_is_matched_on_its_own_deterministic_key() -> None:
    """Matching on anything else — an ObjectId, an insert order — would make a re-run duplicate rather than recognise what it already wrote."""
...
async def test_an_existing_row_is_never_rewritten() -> None:
    """$setOnInsert, not $set: a row already in the ledger is history and must not be restated with today's re-derived numbers."""
    collection = _collection()
    repo = LLMCallsRepository()
...
async def test_one_duplicate_cannot_abort_the_rest_of_the_batch() -> None:
    """A backfill batch is thousands of rows; ordered writes would stop at the first duplicate and silently drop the remainder."""
    collection = _collection()
    repo = LLMCallsRepository()
...
async def test_the_count_returned_is_what_was_actually_created() -> None:
    """Returning the batch size would claim a re-run created rows it recognised and skipped."""
...
async def test_an_empty_batch_does_not_reach_mongo() -> None:
    """The last day of a window is often empty; a bulk_write with no operations raises rather than no-opping."""
    collection = _collection()
    repo = LLMCallsRepository()
...
async def test_the_stored_row_keeps_its_own_creation_time() -> None:
    """created_at is when the call happened, not when the backfill ran — it is the TTL key and the time axis of every ledger query."""
    collection = _collection()
    repo = LLMCallsRepository()
