...
def test_an_executor_thread_yields_the_bare_conversation_and_keeps_the_wrapper() -> None:
    """The bare id joins executor calls to comms calls of the same turn; the wrapper answers "executor lane only"."""
...
# Thread-id shapes seen in production, each minted by a named constructor:
# executor_<conv> (subagent_runner.py:773), <integration>_executor_<conv>
# (handoff_tools.py:507), spawn_<conv>_<call_id> (subagent.py:334), <conv> itself.
...
def test_a_spawned_subagent_thread_resolves_to_its_conversation() -> None:
    """A spawn thread carries no executor_ prefix and used to fall through as a plain, fragmenting conversation id."""
...
def test_two_spawns_in_one_conversation_share_its_conversation_id() -> None:
    """Each spawn has its own thread but they are all spend on the same turn, so a per-conversation cost query must gather them."""
...
def test_every_integration_executor_shape_seen_in_production(integration: str) -> None:
    """Regression guard on 99% of traffic: these wrapped shapes must keep resolving exactly as before."""
...
def test_a_thread_that_merely_starts_with_spawn_is_not_treated_as_wrapped() -> None:
    """A bare spawn_x lacks the trailing tool-call id, so splitting it would invent a conversation that does not exist."""
    assert split_lane_thread("spawn_abc") == ("spawn_abc", None)


def test_an_executor_thread_with_an_underscored_tail_is_left_whole() -> None:
    """No executor constructor appends a tail after the conversation, so such a thread stays unsplit rather than silently wrong."""
...
def test_a_thread_that_merely_mentions_executor_is_not_treated_as_wrapped() -> None:
    """executor_ has to be the prefix of the id, not a substring, or an unrelated thread name gets shredded into a fake conversation."""
...
def test_every_optional_identifier_defaults_to_none_rather_than_a_placeholder() -> None:
    """A system-lane call has no user or conversation; storing "" would make a $ne null query count calls nobody made."""
    doc = _doc()
...
def test_the_document_declares_no_field_that_could_hold_message_text() -> None:
    """Asserted on the field inventory, not an instance, since the risk is a future field being added."""
    banned = {
        "content",
        "text",
        "prompt",
