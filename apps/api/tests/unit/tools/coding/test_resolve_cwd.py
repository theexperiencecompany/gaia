...
def test_workspace_root_cwd_with_session_defaults_to_session_root() -> None:
    cwd, err = _resolve_cwd(WORKSPACE_ROOT, "conv1")
    assert err is None
    assert cwd == session_dir("conv1")


def test_valid_subdir_passes_through_normalized() -> None:
    cwd, err = _resolve_cwd("/workspace/sessions/c/scratch", "c")
    assert err is None
...
def test_relative_cwd_joins_to_session_dir_like_read_and_write() -> None:
    ...
    cwd, err = _resolve_cwd("scratch", "conv1")
...
def test_relative_cwd_without_session_joins_to_workspace_root() -> None:
    cwd, err = _resolve_cwd("sub/dir", None)
    assert err is None
...
def test_relative_cwd_escape_still_rejected() -> None:
    ...
    _, err = _resolve_cwd("../../../../etc", "conv1")
    assert err is not None and "must be under" in err


def test_no_session_no_cwd_resolves_empty_for_workspace_root_fallback() -> None:
    # No session and no cwd: no error, empty cwd; caller falls back to
    # WORKSPACE_ROOT (which always exists — no mkdir needed).
    cwd, err = _resolve_cwd("", None)
    assert err is None
    assert cwd == ""
