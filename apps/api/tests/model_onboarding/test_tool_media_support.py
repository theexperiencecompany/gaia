...
Run this before adding a Concentrate-inference model to
...
Why a live call and not a capability lookup: Concentrate exposes nothing that
answers this. `/v1/models` reports `capabilities.image_input.supported` (whether
the model takes images *at all*) but nothing says whether images survive in a
*tool* message. Two models can be identical there and still disagree — an older
OpenAI-family model 400s on it while a newer one accepts it. So the only honest
test is to send one and see whether the model describes the picture.
...
from app.constants.llm import CONCENTRATE_API_BASE_URL
...
def _seeded_concentrate_models() -> list[str]:
    ...
    return [
        ...
        if model["inference_provider"] == ModelProvider.CONCENTRATE.value
...
def _models_under_test() -> list[str]:
    ...
    return _seeded_concentrate_models()
...
@pytest.mark.skipif(
    not settings.CONCENTRATE_API_KEY, reason="CONCENTRATE_API_KEY is not configured"
)
