...
#: The public backend host the session-directory banner renders. Pinned like
#: every other read here: it comes from settings, so a developer .env pointing
#: HOST at localhost rewrites the banner and fails every recorded snapshot on
#: that machine, while CI (which has no .env) stays green.
SNAPSHOT_HOST = "https://api.heygaia.io"
...
def fake_context_sources(sources: ContextSources) -> Iterator[None]:
    ...
    with ExitStack() as stack:
        ...
        enter(patch("app.utils.artifact_utils.settings.HOST", SNAPSHOT_HOST))
