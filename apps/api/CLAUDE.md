...
- **`app/services/todos/gaia_todo_lifecycle.py` owns every transition and its gate.** Never flip `execution_status` by hand — call `approve` / `dismiss` / `handoff` / `mark_execution_status` / `expire_stale_proposals`.
