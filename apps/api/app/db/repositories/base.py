...
class _BaseRepository(Generic[TDoc, TUpdate]):
    ...
    async def _delete_many(self, filter_: Mapping[str, object], *, scope: str) -> int:
        """Delete every document matching a filter, bumping the scope's generation.

        The filter-based sibling of ``_bulk_delete`` (which deletes by id list) —
        for domain-level wipes like "all of a user's conversations". Returns the
        deleted count."""
