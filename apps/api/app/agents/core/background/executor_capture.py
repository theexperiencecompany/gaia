...
def register_executor_capture(stream_id: str, voice_mode: bool = False) -> asyncio.Event:
    ...
    task can append events to the collector. ``voice_mode`` marks the stream so
    the executor's finalize step publishes a TTS-only ``voice_tts`` frame with
    its narrated answer for the voice agent to speak. Returns the done-event.
    """
    done_event = register_executor_done_event(stream_id)
    register_tool_event_collector(stream_id)
    if voice_mode:
        mark_voice_stream(stream_id)
    return done_event


async def await_executor_done(stream_id: str, timeout: float = EXECUTOR_WAIT_TIMEOUT) -> None:
...
def teardown_executor_capture(stream_id: str) -> None:
    ...
    deregister_voice_stream(stream_id)
