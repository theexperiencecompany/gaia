...
# Pydantic models. These are NOT pickleable — the same restriction the repo
# already documents for Composio tool objects in ``apps/api/CLAUDE.md``.
# Attempting to store these in Redis would raise ``TypeError`` on every
# write. Process-local memo is the only viable option.
#
...
# change. No TTL is applied because an MCP config change is the only
# correctness-relevant invalidation and it's event-driven.
_USER_SUBAGENT_CACHE: dict[tuple[str, str], Any] = {}
...
async def create_subagent(integration_id: str) -> Any:
    ...
    if integration.managed_by == "internal":
        ...
        log.info(f"Internal integration {integration_id}: using pre-registered tools")
    ...
    # Handle MCP-managed integrations (like DeepWiki)
        ...
        if mcp_config.requires_auth:
            ...
            if tools:
                ...
                log.info(f"Registered {len(tools)} MCP tools for {integration_id}")
    ...
    log.set(subagent={"name": config.agent_name, "provider": integration.provider})
    log.info(
...
async def create_subagent_for_user(integration_id: str, user_id: str) -> Any | None:
    ...
    Args:
        integration_id: The integration ID from oauth_config or custom MCP ID
...
async def _build_user_subagent(integration_id: str, user_id: str) -> Any | None:
    ...
    if not integration.subagent_config:
        log.error(f"{integration_id} integration or subagent config not found")
        return None
