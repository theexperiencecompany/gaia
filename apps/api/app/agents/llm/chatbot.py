...
async def chatbot(messages: list[AnyMessage]) -> dict[str, list[BaseMessage]]:
    """Run a one-shot LLM call over a message list (no graph, no checkpointer).

    Always runs on the default model — one-shot helpers never use the pro
    model. Operational failures are logged and re-raised; callers own how
    they degrade, not this helper.
    """
    try:
        response = await ainvoke_llm(resolve_model(), messages, label="chatbot")
