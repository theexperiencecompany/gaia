...
async def chatbot(messages: list[AnyMessage], feature: AIFeature) -> dict[str, list[BaseMessage]]:
    ...
    try:
        response = await ainvoke_llm(get_helper_llm(), messages, label="chatbot", feature=feature)
