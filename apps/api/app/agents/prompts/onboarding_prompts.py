...
2. Inside the executor, after `retrieve_tools(query=...)` returns a list of discovered tool names, you are NOT done. You MUST immediately call `retrieve_tools(exact_tool_names=[...])` with every name the task needs, in ONE call, so they become callable (each extra binding call re-sends the whole conversation instead of resuming from cache). Skipping this step leaves you with no bound tools and you will appear to stall.
