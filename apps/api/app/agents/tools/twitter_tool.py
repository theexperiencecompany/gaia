...
def register_twitter_custom_tools(composio: Composio) -> List[str]:
    ...
    def CUSTOM_BATCH_FOLLOW(
        ...
        try:
            ...
            if not request.usernames and not request.user_ids:
                return {
                    "success": False,
                    "error": "Either usernames or user_ids must be provided",
                }
            ...
            logger.error(f"Error in CUSTOM_BATCH_FOLLOW: {e}")
    ...
    def CUSTOM_BATCH_UNFOLLOW(
        ...
        try:
            ...
            logger.error(f"Error in CUSTOM_BATCH_UNFOLLOW: {e}")
            return {"success": False, "error": str(e)}
        ...
        try:
            ...
            for i, tweet_text in enumerate(request.tweets):
                ...
                if not tweet_id:
                    return {
                        "success": False,
                        "error": f"No ID returned for tweet {i + 1}",
                        "partial_tweet_ids": tweet_ids,
                    }
            ...
            logger.error(f"Error in CUSTOM_CREATE_THREAD: {e}")
            return {"success": False, "error": str(e)}
    ...
    def CUSTOM_SEARCH_USERS(
        ...
        try:
            ...
            if writer is not None:
                writer(
                    {"progress": f"Searching for users matching: {request.query}..."}
                )
            ...
            logger.error(f"Error in CUSTOM_SEARCH_USERS: {e}")
    ...
    def CUSTOM_SCHEDULE_TWEET(
        ...
        try:
            ...
            logger.error(f"Error in CUSTOM_SCHEDULE_TWEET: {e}")
            return {"success": False, "error": str(e)}
