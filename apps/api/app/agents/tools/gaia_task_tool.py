...
async def create_gaia_task(
    ...
    category: Annotated[
        str,
        "Task category: meeting_scheduling, email_follow_up, inbox_management, email_thread_tracking, or general",
    ],
    description: Annotated[str, "Brief description of the task"] = "",
    waiting_for: Annotated[
        Optional[str], "What the task is currently waiting for"
    ] = None,
    watched_thread_ids: Annotated[
        Optional[list[str]], "Gmail thread IDs to monitor for this task"
    ] = None,
    watched_senders: Annotated[
        Optional[list[str]], "Email addresses to monitor (lowercase)"
    ] = None,
    ...
    """Create a persistent task when initiating a multi-step interaction
    that spans sessions (meeting scheduling, email follow-ups, tracking threads).
    Only use for interactions expecting 2+ rounds of back-and-forth.

    The task gets its own conversation and VFS folder.
    """
    ...
    try:
        ...
        try:
            task_category = GaiaTaskCategory(category)
        except ValueError:
            task_category = GaiaTaskCategory.GENERAL
