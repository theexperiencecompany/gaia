...
flight for the workflow; when it runs it drains the whole list into one
batch. ARQ's _job_id dedup collapses the fan-out: the first event schedules
the run, later events in the window are rejected as duplicate enqueues and
survive only as buffered payloads.

Only triggers with a declared poll interval coalesce — a trigger like
calendar_event_starting_soon fires immediately, since a meeting reminder
delayed by its window is a missed meeting.
"""
...
async def buffer_trigger_event(
    ...
    # One id per workflow: while a batch run is queued or executing, every further
    # event's enqueue is deduped away and rides the buffer. `keep_result = 0`
...
async def reschedule_if_refilled(
    ...
    until the next event. Called by the worker at the end of a batched run.
    """
    client = redis_cache.redis
    if client is None or await client.llen(batch_key) == 0:
        return False

    # Renew the buffer's TTL alongside the new job: a workflow rescheduled over and
