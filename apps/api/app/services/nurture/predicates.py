"""Skip predicates for nurture steps: True means the user already does the taught thing.
...
async def onboarding_completed(user: UserDocument) -> bool:  # NOSONAR python:S7503
    """Return True once the user has finished onboarding."""
    return bool(user.onboarding and user.onboarding.completed)


async def used_chat(user: UserDocument) -> bool:
    """Return True once the user has a real (non-onboarding) conversation."""
...
async def google_suite_connected(user: UserDocument) -> bool:
    """Return True only when BOTH Gmail and Google Calendar are connected."""
...
async def uses_todos(user: UserDocument) -> bool:
    """Return True once the user has enough todos to count as using the feature."""
    return await todo_repository.count_for_user(user.id) >= _TODOS_IN_USE_THRESHOLD


async def has_workflow(user: UserDocument) -> bool:
    """Return True once the user has at least one workflow (auto-generated todo workflows count)."""
...
async def linked_platform(user: UserDocument) -> bool:  # NOSONAR python:S7503
    """Return True once the user linked any chat platform (WhatsApp/Telegram/Slack/Discord)."""
