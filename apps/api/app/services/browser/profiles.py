...
encrypted storage_state_blob is never exposed, only the domain and timestamp.
"""
...
async def list_saved_logins(user_id: str) -> list[BrowserLoginResponse]:
    ...
    Each carries expires_at (last use plus the TTL) so the UI can count down to
...
async def forget_saved_login(user_id: str, domain: str | None = None) -> int:
    ...
    primitive so there is one canonical delete path.
    """
