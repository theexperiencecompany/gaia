...
async def _resolve_configured_integrations(
    user_id: str, mongo_status: dict[str, bool]
) -> tuple[dict[str, bool], dict[str, str]]:
    """Status per configured integration, plus the Composio ids still needing a live check."""
    result: dict[str, bool] = {}
    composio_id_to_provider: dict[str, str] = {}
    ...
    return result, composio_id_to_provider
...
async def get_all_integrations_status(user_id: str) -> dict[str, bool]:
    ...
    result, composio_id_to_provider = await _resolve_configured_integrations(user_id, mongo_status)

    # Step 2: Batch check Composio integrations not in MongoDB
    composio_providers = list(composio_id_to_provider.values())
