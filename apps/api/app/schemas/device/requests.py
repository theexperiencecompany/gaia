...
class StartPairingRequest(BaseModel):  # type: ignore[explicit-any]
...
class PollPairingRequest(BaseModel):  # type: ignore[explicit-any]
    """Daemon polls for the user's browser approval."""

    device_code: str = Field(min_length=1)


class ApprovePairingRequest(BaseModel):  # type: ignore[explicit-any]
    """The signed-in browser approves a pairing by its short user_code."""

    user_code: str = Field(min_length=1, max_length=32)


class DeviceTokenRequest(BaseModel):  # type: ignore[explicit-any]
    """Daemon exchanges its refresh credential for a short-lived connect JWT."""

    refresh_token: str = Field(min_length=1)


class RegisterServerRequest(BaseModel):  # type: ignore[explicit-any]
