...
async def generate_user_bio(user_id: str, memories: list[MemoryEntry]) -> tuple[str, BioStatus]:
    log.set(
    ...
    try:
        ...
        profession = user.get("onboarding", {}).get("preferences", {}).get("profession", "")

        # Check Gmail connection status
        composio_service = get_composio_service()
        connection_status = await composio_service.check_connection_status(["gmail"], str(user_id))
