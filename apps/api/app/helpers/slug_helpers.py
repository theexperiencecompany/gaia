...
def generate_workflow_slug(title: str, workflow_id: str, max_length: int = 80) -> str:
    ...
    """
    title_slug = slugify(title, max_length=max_length - 9)  # Reserve 9 chars for -{shortid}
    raw_id = workflow_id.replace("wf_", "") if workflow_id.startswith("wf_") else workflow_id
