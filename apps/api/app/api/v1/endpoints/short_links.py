...
Resolves a heygaia.link slug for the signed-in viewer to the in-app path that
renders the artifact it points at. Viewer-scoped: a slug that is not the
viewer's own resolves to 404, so a forwarded link never exposes someone else's
artifact. The path is returned to the web client (which navigates to it
client-side) rather than issued as a redirect, so it stays host-relative and
...
def _target_path(link: ShortLink) -> str:
    """In-app path that renders the artifact behind ``link``.
...
async def resolve_short_link_route(
    ...
    """Resolve ``slug`` for the signed-in viewer to its in-app artifact path."""
    ...
    return JSONResponse(content={"path": _target_path(link)})
