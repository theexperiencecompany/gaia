...
async def websocket_endpoint(websocket: WebSocket):
    ...
    LoggingMiddleware never dispatches websocket scope, so this boundary is
    what emits the connection's wide event — one ``ws_connection`` line per
    connection lifetime, covering auth failures too. A normal disconnect is
    caught inside the boundary, so it exits with outcome=success.
    """
    async with log_context("ws_connection"):
