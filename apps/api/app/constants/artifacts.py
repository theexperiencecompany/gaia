...
# Mirrors GET /api/v1/sessions/{conv_id}/artifacts/{path}; single source of
# truth for the save-time relative->absolute rewrite and the agent-facing session banner.
...
# seeding uploads anyway. Pubsub has no replay (an event published before the
# subscription exists is dropped), so past the timeout files on disk win over event delivery.
