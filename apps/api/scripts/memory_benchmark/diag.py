...
async def _diag_recall(user_id: str, query: str) -> None:
    ...
    print(f"    FTS hits: {[(str(i)[:8], round(s, 3)) for i, s in fts_hits[:5]]}")
    if ann_hits:
        from app.memory import pg_store as _pg

        rows = await _pg.get_memories_by_ids(user_id, [str(m) for m, _ in ann_hits])
        print(
            f"    pg rows for ANN ids: {[(str(r.id)[:8], r.is_latest, r.is_forgotten) for r in rows]}"
        )
        recent = await _pg.get_recent_facts(user_id, limit=50)
        print(f"    pg recent facts: {len(recent)}")
    fused = ret._rrf_fuse([mid for mid, _ in ann_hits], [str(i) for i, _ in fts_hits])
    ...
    for c, s in scored[:6]:
        print(f"    rerank {c.memory.id[:8]} -> {round(s, 4)} | {c.memory.content[:70]!r}")
...
async def main() -> None:
    ...
    from scripts.evals.suites.memory import (
    ...
    for scenario in SCENARIOS:
        ...
        try:
            ...
            for m in stored.memories:
                print(f"    {m.id[:8]} | {m.content[:90]!r} | kind={m.kind} cat={m.category_path}")
