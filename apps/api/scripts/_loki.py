...
def nanos(moment: datetime) -> int:
    """Return moment as whole nanoseconds — Loki's own timestamp resolution.

    Deliberately integer: rounding the page cursor to a second, or through a
    datetime's float seconds, restarts the next page inside a second already
    returned, folding every event in it twice and inflating totals for the
    busiest days.
    """
...
class PageBudgetExhaustedError(RuntimeError):
    """A day exceeding LOKI_MAX_PAGES pages leaves only a prefix, which must not be written."""
...
class UndrainableTimestampError(RuntimeError):
    """More lines share one nanosecond than fit in a Loki page, so the day can't be read completely."""
...
async def fetch_day(
    ...
    """Return every llm_call event Loki holds for one UTC day, parsed by parse.

    Pages forward since Loki caps a response and has no cursor. The next
    page re-opens AT the last timestamp seen, skipping lines already taken
    from it by identity — one nanosecond later drops the rest of that group,
    coarser double-counts. parse returns None for a line to drop.
    """
...
async def _drain_timestamp(
    ...
    """Read everything logged at at_nanos, keeping what wasn't taken on an earlier page.

    Returns False when the group is larger than one page, so Loki can't
    serve it whole and the day can't be read completely.
    """
...
class _PageScan:
    """What one Loki page contributed.

    How many lines it held, how many were new, and the lines at its last
    nanosecond WITH their multiplicity — two identical lines can share a
    timestamp, and the next page must skip exactly the ones already taken,
    not every line that looks like them.
    """
