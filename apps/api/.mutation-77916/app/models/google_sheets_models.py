...
class PivotValue(BaseModel):
    ...
    aggregation: Literal["SUM", "AVERAGE", "COUNT", "COUNTA", "MAX", "MIN"] = Field(
        default="SUM", description="Aggregation function"
    )
    name: str | None = Field(
        default=None, description="Custom display name for this value (optional)"
    )
...
class CreatePivotTableInput(BaseModel):
    ...
    destination_cell: str = Field(
        default="A1", description="Cell where pivot table starts (e.g., 'A1')"
    )
    rows: list[str] = Field(
        ...,
...
class DataValidationInput(BaseModel):
    ...
    values: list[str] | None = Field(
        default=None, description="List of allowed values for dropdown_list type"
    )
    ...
    min_value: str | None = Field(
        default=None, description="Minimum allowed value for number/date validation"
    )
    max_value: str | None = Field(
        default=None, description="Maximum allowed value for number/date validation"
    )

    # For custom_formula
    formula: str | None = Field(
        default=None, description="Custom formula for validation (e.g., '=LEN(A1)<=50')"
    )

    # Common options
    show_dropdown: bool = Field(
        default=True, description="Show dropdown arrow in cell (for dropdown types)"
    )
    input_message: str | None = Field(
        default=None, description="Help message shown when cell is selected"
    )
    strict: bool = Field(
        default=True, description="Reject invalid input if True, warn only if False"
    )
...
class ConditionalFormatInput(BaseModel):
    ...
    condition: (
        ...
        | None
    ) = Field(default=None, description="Condition type for value_based formatting")
    ...
    background_color: str | None = Field(
        default=None, description="Background color in hex (e.g., '#FF0000' for red)"
    )
    text_color: str | None = Field(
        default=None, description="Text color in hex (e.g., '#FFFFFF' for white)"
    )
    bold: bool | None = Field(default=None, description="Make text bold")
    italic: bool | None = Field(default=None, description="Make text italic")

    # For color_scale gradient
    min_color: str | None = Field(
        default=None, description="Color for minimum values in color_scale (hex)"
    )
    mid_color: str | None = Field(
        default=None, description="Color for middle values in color_scale (hex, optional)"
    )
    max_color: str | None = Field(
        default=None, description="Color for maximum values in color_scale (hex)"
    )
...
class ChartInput(BaseModel):
    ...
    x_axis_title: str | None = Field(default=None, description="Title for X axis")
    y_axis_title: str | None = Field(default=None, description="Title for Y axis")
    ...
    anchor_cell: str = Field(
        default="E1", description="Cell where top-left of chart anchors (e.g., 'E1')"
    )
    width: int = Field(default=600, description="Chart width in pixels", ge=100, le=2000)
    height: int = Field(default=400, description="Chart height in pixels", ge=100, le=2000)
    legend_position: Literal[
        "BOTTOM_LEGEND", "LEFT_LEGEND", "RIGHT_LEGEND", "TOP_LEGEND", "NO_LEGEND"
    ] = Field(default="BOTTOM_LEGEND", description="Position of chart legend")
