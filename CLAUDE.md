...
- **Docker layer cache is `type=gha` scoped with `scope=` per image.** `cache-from: type=gha,scope=<image>` + `cache-to: type=gha,mode=max,scope=<image>` (e.g. `scope=web` vs `scope=grafana`). Without `scope=` all jobs share one 10 GB GHA blob and thrash each other. `mode=max` preserves layers; `mode=min` alone evicts.
