...
- **Never Nx Cloud (`NX_NO_CLOUD=true`).** There is no remote Nx cache on GitHub-hosted lanes: `.nx/cache` is local to the job. On the home box the workspace and its `.nx/cache` persist between jobs, plus the shared Nx cache service in `infra/self-hosted-runner/nx-cache-server/`. Where a cache IS keyed through `actions/cache` (Next.js compiler cache, uv), key on a hash — never `restore-keys` without one.
...
- `main.yml: trivy-scan` runs `scan-type: fs`, `severity: CRITICAL,HIGH`, `scan-ref: .`, `format: table`, `ignore-unfixed: false`, and **`exit-code: "0"` — advisory, it cannot red a PR**. On the home box it sets `skip-setup-trivy: true` (the pinned binary is installed by `infra/self-hosted-runner/setup.sh`), keeps the vuln DB on local disk instead of `actions/cache`, and `skip-dirs` the persistent `node_modules`/`.venv`/`.nx` trees — lockfiles already carry the dependency truth. The job is nonetheless in `quality-gate.needs`, so the gate fails if the job itself errors or is cancelled; findings alone do not.
