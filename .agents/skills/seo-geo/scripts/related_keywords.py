...
def main():
    ...
    parser.add_argument(
        "--location", "-loc", type=int, default=2840, help="Location code (default: 2840 = US)"
    )
    parser.add_argument(
        "--depth",
        "-d",
    ...
    parser.add_argument(
        "--limit", "-l", type=int, default=50, help="Max results to display (default: 50)"
    )
    ...
    if results:
        ...
        for result in results:
            ...
            for item in items:
                ...
                difficulty = kw_data.get(
                    "keyword_difficulty", item.get("keyword_difficulty", "N/A")
                )
        ...
        print(
            f"related_keywords[{len(display_keywords)} of {len(keywords)}]{{keyword,volume,difficulty}}:"
        )
