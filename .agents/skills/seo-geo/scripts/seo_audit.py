...
def fetch_url(url: str, timeout: int = 30) -> tuple:
    ...
    # The audited URL comes from argv, so the scheme has to be checked: urlopen
    # happily serves file:// and would turn "audit this page" into reading local
    # files. An SEO audit only ever means http(s).
    if urllib.parse.urlparse(url).scheme not in ("http", "https"):
        print(f"error: refusing non-http(s) URL: {url}", file=sys.stderr)
...
def main():
    ...
    title = meta["title"]
    print(
        f"title: {title[:60] if title else 'MISSING'}{'...' if title and len(title) > 60 else ''}"
    )
    print(f"title_length: {len(title) if title else 0} chars")
    desc = meta["description"]
    print(
        f"description: {desc[:80] if desc else 'MISSING'}{'...' if desc and len(desc) > 80 else ''}"
    )
