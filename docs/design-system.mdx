...
<Warning>
  Never use `rounded-lg` on card containers, that's the Shadcn base radius, visually too small. Chat outer cards always use `rounded-3xl`; inner items and settings surfaces use `rounded-2xl`.
</Warning>
...
<div style={{ background: '#0d0d0d', padding: '28px', borderRadius: '12px', margin: '16px 0' }}>
  <div style={{ borderRadius: '24px', background: '#27272a', padding: '16px', maxWidth: '380px' }}>
...
| Outer (accordion) | `rounded-3xl bg-zinc-800 p-3 py-0` |
