...
<CardGroup cols={2}>
  <Card title="Core Application">
    ```bash ENV=production DEBUG=false HOST=https://api.heygaia.io
    FRONTEND_URL=https://heygaia.io ```
  </Card>
  <Card title="Database URLs">
    ```bash POSTGRES_URL=postgresql://user:pass@host:5432/db
    MONGO_DB=mongodb://user:pass@host:27017/gaia
    REDIS_URL=redis://user:pass@host:6379 ```
  </Card>
  <Card title="AI API Keys">
    ```bash OPENAI_API_KEY=sk-your-openai-key GEMINI_API_KEY=your-gemini-key
    HUGGINGFACE_API_KEY=hf_your-hf-token ```
  </Card>
  <Card title="Integration APIs">
    ```bash GOOGLE_CLIENT_ID=your-google-client-id
    GOOGLE_CLIENT_SECRET=your-google-secret TAVILY_API_KEY=your-tavily-key
...
<CardGroup cols={2}>
  <Card title="Access Control">
    ...
    required permissions - Regularly audit access logs
  </Card>
  <Card title="Secret Management">
    - Use different secrets for dev/staging/production - Rotate secrets
    regularly - Never commit Infisical credentials to version control
  </Card>
  <Card title="Environment Separation">
    - Use separate Infisical projects for each environment - Implement proper
    CI/CD secret injection - Monitor secret access patterns
  </Card>
