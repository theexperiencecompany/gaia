...
The Python services and the TypeScript bots emit the **same key names for the
same concepts**, so a single LogQL query spans both: `time`, `level` (loguru
names — `WARNING`, not `WARN`), `service`, `logger`, `message`, `env`,
`trace_id`, `duration_ms`, `outcome`, `final_level`, `commit`, and the
`errors[]` / `warnings[]` / `audit[]` arrays (each entry keyed by `msg`).
...
`http_request` for Python, `bot_event` for the bots.
