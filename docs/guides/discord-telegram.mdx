...
<Note>
  For a complete command reference including subcommand options and examples,
  see the [Bot Commands Reference](/bots/commands).
</Note>
...
<Tip>
  "Summarize with GAIA" and "Add as Todo" work on any message in the server —
  not just your own. Use them to pull action items out of team discussions
...
<Warning>
  Your GAIA account must be linked via `/auth` before most commands work. If a
  command returns an authentication error, run `/auth` and complete the sign-in
  flow.
</Warning>
...
<CardGroup cols={2}>
  ...
  >
    Link Gmail, Calendar, GitHub, and 60+ other tools so GAIA can act on your
    behalf.
  </Card>
  <Card
    title="Build a Workflow"
    icon="bolt"
    href="/guides/marketplace#publishing-a-workflow"
  >
  ...
  <Card
    ...
    icon="comments"
    href="/guides/chatting-effectively"
  >
    Learn prompting techniques that help GAIA give better, more actionable
    answers.
  </Card>
