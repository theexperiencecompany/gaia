  ...
  useFrame((_, delta: number) => {
    ...
    if (modeRef.current === "manual") {
      ...
      if (agentRef.current === null) {
        // Idle state - very minimal, slow pulsing
        targetIn = 0.05 + 0.05 * Math.sin(t * 0.8);
        targetOut = 0.25 + 0.05 * Math.sin(t * 0.6);
      } else if (agentRef.current === "listening") {
        targetIn = clamp01(0.55 + Math.sin(t * 3.2) * 0.35);
        targetOut = 0.45 + 0.1 * Math.sin(t * 2.8);
    ...
    // State-dependent animation speed for more behavioral differentiation
    let baseSpeed = 0.1;
    if (agentRef.current === null) {
      // Idle - slower animation for calm appearance
      baseSpeed = 0.06;
    } else if (agentRef.current === "listening") {
