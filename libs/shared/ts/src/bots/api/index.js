...
export class GaiaClient {
  ...
  async chat(request) {
    try {
      const { data } = await this.client.post("/api/v1/bot/chat", {
   ...
   * @throws Error if the API request fails.
   */
  async getSession(platform, platformUserId, channelId) {
   ...
   * @throws Error if the API request fails.
   */
  async checkAuthStatus(platform, platformUserId) {
    try {
      const { data } = await this.client.get(
   ...
   */
  getAuthUrl(platform, platformUserId) {
    const params = new URLSearchParams({
      platform,
