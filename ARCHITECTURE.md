...
- `apps/api/app/agents/tools/execute/schema_docs.py` — compact budgeted schema docs. Response shapes rendered only when the provider supplies one — `wrap_tool` (langchain_composio_service.py) copies Composio's `output_parameters` onto `tool.metadata` to feed this. Args and Returns each have their own char budget; an oversized schema depth-prunes progressively (nested detail collapses to a visible `"..."` marker) and floors at field names, never clipped mid-JSON.
