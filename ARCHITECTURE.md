...
- `apps/api/app/agents/tools/execute/schema_docs.py` — compact budgeted schema docs. Response shapes are never invented: provider-supplied (`wrap_tool` copies Composio's `output_parameters` onto `tool.metadata`) or **observed** (`app/services/tool_shape_service.py` + `tool_output_shapes` Mongo collection — every successful integration dispatch fire-and-forgets a genson merge of the real output's structure; keys/types only, arrays sampled, wide dicts collapsed to maps, never values). Args depth-prune inline (never clipped mid-JSON); Returns inlines only under `RESPONSE_SCHEMA_INLINE_MAX_CHARS`, else one pointer line (top-level field names + how to fetch the full schema).
- `apps/api/app/agents/tools/execute/tool_info.py` + `POST /api/v1/sandbox/tool-schema` — the depth behind the pointer: full input/provider/observed schemas + `observed_call_count`. Sandbox scripts call `gaia.schema("TOOL")`, cached one-file-per-tool at `/tmp/.gaia/tools/` (TTL cache of the host store; becomes a shared-mount symlink when E2B<->JuiceFS is reliable). Token-authed, shares the execute budget.
