import React from "react";
import { HugeiconsIcon } from "@hugeicons/react";
import {
  AccountSetting02Icon as AccountSetting02IconSource,
  AiBrain01Icon as AiBrain01IconSource,
  AiBrowserIcon as AiBrowserIconSource,
  AiImageIcon as AiImageIconSource,
  AiSearch02Icon as AiSearch02IconSource,
  AiWebBrowsingIcon as AiWebBrowsingIconSource,
  Alert01Icon as Alert01IconSource,
  Alert02Icon as Alert02IconSource,
  AppleIcon as AppleIconSource,
  Appointment01Icon as Appointment01IconSource,
  ArrowLeft01Icon as ArrowLeft01IconSource,
  ArrowRight01Icon as ArrowRight01IconSource,
  ArrowUpRight01Icon as ArrowUpRight01IconSource,
  AttachmentIcon as AttachmentIconSource,
  BlushBrush02Icon as BlushBrush02IconSource,
  BookOpen01Icon as BookOpen01IconSource,
  BookOpen02Icon as BookOpen02IconSource,
  Brain02Icon as Brain02IconSource,
  BrushIcon as BrushIconSource,
  BubbleChatIcon as BubbleChatIconSource,
  BubbleChatLockIcon as BubbleChatLockIconSource,
  BubbleChatQuestionIcon as BubbleChatQuestionIconSource,
  Calendar01Icon as Calendar01IconSource,
  Calendar03Icon as Calendar03IconSource,
  CalendarAdd01Icon as CalendarAdd01IconSource,
  CalendarCheckOut02Icon as CalendarCheckOut02IconSource,
  CalendarIcon as CalendarIconSource,
  Call02Icon as Call02IconSource,
  Cancel01Icon as Cancel01IconSource,
  ChatBotIcon as ChatBotIconSource,
  CheckmarkBadge01Icon as CheckmarkBadge01IconSource,
  CheckmarkCircle02Icon as CheckmarkCircle02IconSource,
  CheckmarkSquare03Icon as CheckmarkSquare03IconSource,
  CloudAngledRainIcon as CloudAngledRainIconSource,
  CloudAngledZapIcon as CloudAngledZapIconSource,
  CloudIcon as CloudIconSource,
  CloudLittleRainIcon as CloudLittleRainIconSource,
  CloudSnowIcon as CloudSnowIconSource,
  CommentAdd01Icon as CommentAdd01IconSource,
  ComputerIcon as ComputerIconSource,
  ComputerPhoneSyncIcon as ComputerPhoneSyncIconSource,
  ConnectIcon as ConnectIconSource,
  CreditCardIcon as CreditCardIconSource,
  CreditCardPosIcon as CreditCardPosIconSource,
  CursorMagicSelection03Icon as CursorMagicIconSource,
  CustomerService01Icon as CustomerService01IconSource,
  DashboardSquare01Icon as DashboardSquare01IconSource,
  DiscoverCircleIcon as DiscoverCircleIconSource,
  DocumentAttachmentIcon as DocumentAttachmentIconSource,
  DownloadSquare01Icon as DownloadSquare01IconSource,
  DropletIcon as DropletIconSource,
  FallingStarIcon as FallingStarIconSource,
  FastWindIcon as FastWindIconSource,
  FileEmpty02Icon as FileEmpty02IconSource,
  FileUploadIcon as FileUploadIconSource,
  FlowchartIcon as FlowchartIconSource,
  Folder02Icon as Folder02IconSource,
  GlobalIcon as GlobalIconSource,
  GlobalSearchIcon as GlobalSearchIconSource,
  GoogleIcon as GoogleIconSource,
  HelpCircleIcon as HelpCircleIconSource,
  Home01Icon as Home01IconSource,
  Idea01Icon as Idea01IconSource,
  IdentityCardIcon as IdentityCardIconSource,
  Image02Icon as Image02IconSource,
  ImageAdd02Icon as ImageAdd02IconSource,
  ImageUploadIcon as ImageUploadIconSource,
  InboxIcon as InboxIconSource,
  InternetIcon as InternetIconSource,
  LabelImportantIcon as LabelImportantIconSource,
  LanguageSkillIcon as LanguageSkillIconSource,
  LicenseDraftIcon as LicenseDraftIconSource,
  LinkBackwardIcon as LinkBackwardIconSource,
  LinkedinIcon as LinkedinIconSource,
  LockPasswordIcon as LockPasswordIconSource,
  Logout02Icon as Logout02IconSource,
  MagicWand05Icon as MagicWand05IconSource,
  Mail01Icon as Mail01IconSource,
  MapsIcon as MapsIconSource,
  Menu01Icon as Menu01IconSource,
  Menu02Icon as Menu02IconSource,
  MessageAdd01Icon as MessageAdd01IconSource,
  MessageFavourite02Icon as MessageFavourite02IconSource,
  MessageMultiple02Icon as MessageMultiple02IconSource,
  Mic01Icon as Mic01IconSource,
  Mic02Icon as Mic02IconSource,
  MoneyBag02Icon as MoneyBag02IconSource,
  Moon02Icon as Moon02IconSource,
  NewsIcon as NewsIconSource,
  NoteDoneIcon as NoteDoneIconSource,
  NotificationIcon as NotificationIconSource,
  PackageOpenIcon as PackageOpenIconSource,
  PasswordValidationIcon as PasswordValidationIconSource,
  Pdf02Icon as Pdf02IconSource,
  PencilEdit01Icon as PencilEdit01IconSource,
  PencilEdit02Icon as PencilEdit02IconSource,
  PinIcon as PinIconSource,
  PlayIcon as PlayIconSource,
  PlusSignIcon as PlusSignIconSource,
  QuillWrite01Icon as QuillWrite01IconSource,
  RedditIcon as RedditIconSource,
  RemoveCircleIcon as RemoveCircleIconSource,
  Route02Icon as Route02IconSource,
  SearchIcon as SearchIconSource,
  Sent02Icon as Sent02IconSource,
  SentIcon as SentIconSource,
  Settings01Icon as Settings01IconSource,
  Share05Icon as Share05IconSource,
  SidebarLeft01Icon as SidebarLeft01IconSource,
  SidebarLeftIcon as SidebarLeftIconSource,
  SidebarRight01Icon as SidebarRight01IconSource,
  SmartPhone01Icon as SmartPhone01IconSource,
  SourceCodeCircleIcon as SourceCodeCircleIconSource,
  SquareLock02Icon as SquareLock02IconSource,
  StarsIcon as StarsIconSource,
  StickyNote01Icon as StickyNote01IconSource,
  Sun03Icon as Sun03IconSource,
  Target02Icon as Target02IconSource,
  Task01Icon as Task01IconSource,
  TaskDone01Icon as TaskDone01IconSource,
  ThermometerWarmIcon as ThermometerWarmIconSource,
  Tick02Icon as Tick02IconSource,
  TimeScheduleIcon as TimeScheduleIconSource,
  Timer02Icon as Timer02IconSource,
  ToolsIcon as ToolsIconSource,
  Tornado02Icon as Tornado02IconSource,
  TranslateIcon as TranslateIconSource,
  UserCircle02Icon as UserCircle02IconSource,
  UserCircleIcon as UserCircleIconSource,
  UserIcon as UserIconSource,
  Video01Icon as Video01IconSource,
  ViewIcon as ViewIconSource,
  ViewOffSlashIcon as ViewOffSlashIconSource,
  VisionIcon as VisionIconSource,
  VoiceIcon as VoiceIconSource,
  VolumeHighIcon as VolumeHighIconSource,
  VolumeOffIcon as VolumeOffIconSource,
  Watch02Icon as Watch02IconSource,
  WavingHand01Icon as WavingHand01IconSource,
  WorkflowSquare03Icon as WorkflowSquare03IconSource,
  ZapIcon as ZapIconSource,
} from "@hugeicons/core-free-icons";

export interface IconProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  strokeWidth?: number;
}

const createIcon = (icon: any) => {
  const IconComponent: React.FC<IconProps> = ({
    size = 24,
    color = "currentColor",
    strokeWidth = 1.5,
    ...props
  }) => (
    <HugeiconsIcon
      icon={icon}
      size={size}
      color={color}
      strokeWidth={strokeWidth}
      {...props}
    />
  );
  return IconComponent;
};

export const GlobalIcon = createIcon(GlobalIconSource);
export const PinIcon = createIcon(PinIconSource);
export const DiscoverCircleIcon = createIcon(DiscoverCircleIconSource);
export const CalendarIcon = createIcon(CalendarIconSource);
export const Calendar01Icon = createIcon(Calendar01IconSource);
export const SearchIcon = createIcon(SearchIconSource);
export const StarsIcon = createIcon(StarsIconSource);
export const PlusSignIcon = createIcon(PlusSignIconSource);
export const PencilEdit02Icon = createIcon(PencilEdit02IconSource);
export const UserCircleIcon = createIcon(UserCircleIconSource);
export const Settings01Icon = createIcon(Settings01IconSource);
export const BlushBrush02Icon = createIcon(BlushBrush02IconSource);
export const Logout02Icon = createIcon(Logout02IconSource);
export const SentIcon = createIcon(SentIconSource);
export const AttachmentIcon = createIcon(AttachmentIconSource);
export const GlobalSearchIcon = createIcon(GlobalSearchIconSource);
export const WavingHand01Icon = createIcon(WavingHand01IconSource);
export const SidebarLeftIcon = createIcon(SidebarLeftIconSource);
export const ChatBotIcon = createIcon(ChatBotIconSource);
export const Task01Icon = createIcon(Task01IconSource);
export const Menu02Icon = createIcon(Menu02IconSource);
export const TranslateIcon = createIcon(TranslateIconSource);
export const Pdf02Icon = createIcon(Pdf02IconSource);
export const Image02Icon = createIcon(Image02IconSource);
export const Mic01Icon = createIcon(Mic01IconSource);
export const VoiceIcon = createIcon(VoiceIconSource);
export const Mic02Icon = createIcon(Mic02IconSource);
export const Tick02Icon = createIcon(Tick02IconSource);
export const Cancel01Icon = createIcon(Cancel01IconSource);
export const CalendarAdd01Icon = createIcon(CalendarAdd01IconSource);
export const GoogleIcon = createIcon(GoogleIconSource);
export const Timer02Icon = createIcon(Timer02IconSource);
export const WorkflowSquare03Icon = createIcon(WorkflowSquare03IconSource);
export const Target02Icon = createIcon(Target02IconSource);
export const LanguageSkillIcon = createIcon(LanguageSkillIconSource);
export const NoteDoneIcon = createIcon(NoteDoneIconSource);
export const Share05Icon = createIcon(Share05IconSource);
export const DocumentAttachmentIcon = createIcon(DocumentAttachmentIconSource);
export const LinkBackwardIcon = createIcon(LinkBackwardIconSource);
export const ArrowUpRight01Icon = createIcon(ArrowUpRight01IconSource);
export const FallingStarIcon = createIcon(FallingStarIconSource);
export const SmartPhone01Icon = createIcon(SmartPhone01IconSource);
export const ComputerIcon = createIcon(ComputerIconSource);
export const Watch02Icon = createIcon(Watch02IconSource);
export const Mail01Icon = createIcon(Mail01IconSource);
export const SquareLock02Icon = createIcon(SquareLock02IconSource);
export const CommentAdd01Icon = createIcon(CommentAdd01IconSource);
export const Menu01Icon = createIcon(Menu01IconSource);
export const Home01Icon = createIcon(Home01IconSource);
export const CheckmarkBadge01Icon = createIcon(CheckmarkBadge01IconSource);
export const ArrowRight01Icon = createIcon(ArrowRight01IconSource);
export const ArrowLeft01Icon = createIcon(ArrowLeft01IconSource);
export const SourceCodeCircleIcon = createIcon(SourceCodeCircleIconSource);
export const ComputerPhoneSyncIcon = createIcon(ComputerPhoneSyncIconSource);
export const AccountSetting02Icon = createIcon(AccountSetting02IconSource);
export const FileUploadIcon = createIcon(FileUploadIconSource);
export const InternetIcon = createIcon(InternetIconSource);
export const ImageAdd02Icon = createIcon(ImageAdd02IconSource);
export const BrushIcon = createIcon(BrushIconSource);
export const DownloadSquare01Icon = createIcon(DownloadSquare01IconSource);
export const VolumeHighIcon = createIcon(VolumeHighIconSource);
export const ImageUploadIcon = createIcon(ImageUploadIconSource);
export const AiImageIcon = createIcon(AiImageIconSource);
export const Alert01Icon = createIcon(Alert01IconSource);
export const ViewOffSlashIcon = createIcon(ViewOffSlashIconSource);
export const ViewIcon = createIcon(ViewIconSource);
export const LockPasswordIcon = createIcon(LockPasswordIconSource);
export const IdentityCardIcon = createIcon(IdentityCardIconSource);
export const Alert02Icon = createIcon(Alert02IconSource);
export const PlayIcon = createIcon(PlayIconSource);
export const BubbleChatIcon = createIcon(BubbleChatIconSource);
export const UserIcon = createIcon(UserIconSource);
export const MoneyBag02Icon = createIcon(MoneyBag02IconSource);
export const HelpCircleIcon = createIcon(HelpCircleIconSource);
export const BubbleChatQuestionIcon = createIcon(BubbleChatQuestionIconSource);
export const CheckmarkCircle02Icon = createIcon(CheckmarkCircle02IconSource);
export const PasswordValidationIcon = createIcon(PasswordValidationIconSource);
export const TaskDone01Icon = createIcon(TaskDone01IconSource);
export const StickyNote01Icon = createIcon(StickyNote01IconSource);
export const Route02Icon = createIcon(Route02IconSource);
export const VolumeOffIcon = createIcon(VolumeOffIconSource);
export const FlowchartIcon = createIcon(FlowchartIconSource);
export const BubbleChatLockIcon = createIcon(BubbleChatLockIconSource);
export const NotificationIcon = createIcon(NotificationIconSource);
export const Brain02Icon = createIcon(Brain02IconSource);
export const AiBrowserIcon = createIcon(AiBrowserIconSource);
export const CheckmarkSquare03Icon = createIcon(CheckmarkSquare03IconSource);
export const InboxIcon = createIcon(InboxIconSource);
export const Sent02Icon = createIcon(Sent02IconSource);
export const LicenseDraftIcon = createIcon(LicenseDraftIconSource);
export const QuillWrite01Icon = createIcon(QuillWrite01IconSource);
export const LabelImportantIcon = createIcon(LabelImportantIconSource);
export const TimeScheduleIcon = createIcon(TimeScheduleIconSource);
export const AiSearch02Icon = createIcon(AiSearch02IconSource);
export const MagicWand05Icon = createIcon(MagicWand05IconSource);
export const MapsIcon = createIcon(MapsIconSource);
export const Idea01Icon = createIcon(Idea01IconSource);
export const BookOpen02Icon = createIcon(BookOpen02IconSource);
export const CustomerService01Icon = createIcon(CustomerService01IconSource);
export const CreditCardPosIcon = createIcon(CreditCardPosIconSource);
export const NewsIcon = createIcon(NewsIconSource);
export const Video01Icon = createIcon(Video01IconSource);
export const PencilEdit01Icon = createIcon(PencilEdit01IconSource);
export const UserCircle02Icon = createIcon(UserCircle02IconSource);
export const CloudAngledZapIcon = createIcon(CloudAngledZapIconSource);
export const CloudSnowIcon = createIcon(CloudSnowIconSource);
export const CloudAngledRainIcon = createIcon(CloudAngledRainIconSource);
export const CloudIcon = createIcon(CloudIconSource);
export const Sun03Icon = createIcon(Sun03IconSource);
export const CloudLittleRainIcon = createIcon(CloudLittleRainIconSource);
export const Moon02Icon = createIcon(Moon02IconSource);
export const FastWindIcon = createIcon(FastWindIconSource);
export const DropletIcon = createIcon(DropletIconSource);
export const ThermometerWarmIcon = createIcon(ThermometerWarmIconSource);
export const VisionIcon = createIcon(VisionIconSource);
export const Tornado02Icon = createIcon(Tornado02IconSource);
export const AppleIcon = createIcon(AppleIconSource);
export const Call02Icon = createIcon(Call02IconSource);
export const AiWebBrowsingIcon = createIcon(AiWebBrowsingIconSource);
export const Folder02Icon = createIcon(Folder02IconSource);
export const Appointment01Icon = createIcon(Appointment01IconSource);
export const CalendarCheckOut02Icon = createIcon(CalendarCheckOut02IconSource);
export const Calendar03Icon = createIcon(Calendar03IconSource);
export const SidebarLeft01Icon = createIcon(SidebarLeft01IconSource);
export const SidebarRight01Icon = createIcon(SidebarRight01IconSource);
export const AiBrain01Icon = createIcon(AiBrain01IconSource);
export const MessageMultiple02Icon = createIcon(MessageMultiple02IconSource);
export const DashboardSquare01Icon = createIcon(DashboardSquare01IconSource);
export const FileEmpty02Icon = createIcon(FileEmpty02IconSource);
export const ToolsIcon = createIcon(ToolsIconSource);
export const CreditCardIcon = createIcon(CreditCardIconSource);
export const ConnectIcon = createIcon(ConnectIconSource);
export const ZapIcon = createIcon(ZapIconSource);
export const PackageOpenIcon = createIcon(PackageOpenIconSource);
export const RemoveCircleIcon = createIcon(RemoveCircleIconSource);
export const MessageAdd01Icon = createIcon(MessageAdd01IconSource);
export const BookOpen01Icon = createIcon(BookOpen01IconSource);
export const RedditIcon = createIcon(RedditIconSource);
export const MessageFavourite02Icon = createIcon(MessageFavourite02IconSource);
export const CursorMagicSelection03Icon = createIcon(CursorMagicIconSource);

export const GoogleColouredIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    color={"currentColor"}
    fill={"none"}
    height={24}
    viewBox="0 0 24 24"
    width={24}
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      fill="#4285F4"
    />
    <path
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      fill="#34A853"
    />
    <path
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      fill="#FBBC05"
    />
    <path
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      fill="#EA4335"
    />
    <path d="M1 1h22v22H1z" fill="none" />
  </svg>
);

export const ChatBubbleAddIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    color={"currentColor"}
    fill={"none"}
    height={24}
    viewBox="0 0 24 24"
    width={24}
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M12.4996 14.5V8.5M9.49962 11.5H15.4996M12.4996 20C17.194 20 20.9996 16.1944 20.9996 11.5C20.9996 6.80558 17.194 3 12.4996 3C7.8052 3 3.99962 6.80558 3.99962 11.5C3.99962 12.45 4.15547 13.3636 4.443 14.2166C4.55119 14.5376 4.60529 14.6981 4.61505 14.8214C4.62469 14.9432 4.6174 15.0286 4.58728 15.1469C4.55677 15.2668 4.48942 15.3915 4.35472 15.6408L2.71906 18.6684C2.48575 19.1002 2.36909 19.3161 2.3952 19.4828C2.41794 19.6279 2.50337 19.7557 2.6288 19.8322C2.7728 19.9201 3.01692 19.8948 3.50517 19.8444L8.62619 19.315C8.78127 19.299 8.85881 19.291 8.92949 19.2937C8.999 19.2963 9.04807 19.3029 9.11586 19.3185C9.18478 19.3344 9.27145 19.3678 9.44478 19.4345C10.3928 19.7998 11.4228 20 12.4996 20Z"
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    />
  </svg>
);

export const ToolIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    width="100%"
    height="100%"
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M6 6L10.5 10.5M6 6H3L2 3L3 2L6 3V6ZM19.259 2.74101L16.6314 5.36863C16.2354 5.76465 16.0373 5.96265 15.9632 6.19098C15.8979 6.39183 15.8979 6.60817 15.9632 6.80902C16.0373 7.03735 16.2354 7.23535 16.6314 7.63137L16.8686 7.86863C17.2646 8.26465 17.4627 8.46265 17.691 8.53684C17.8918 8.6021 18.1082 8.6021 18.309 8.53684C18.5373 8.46265 18.7354 8.26465 19.1314 7.86863L21.5893 5.41072C21.854 6.05488 22 6.76039 22 7.5C22 10.5376 19.5376 13 16.5 13C16.1338 13 15.7759 12.9642 15.4298 12.8959C14.9436 12.8001 14.7005 12.7521 14.5532 12.7668C14.3965 12.7824 14.3193 12.8059 14.1805 12.8802C14.0499 12.9501 13.919 13.081 13.657 13.343L6.5 20.5C5.67157 21.3284 4.32843 21.3284 3.5 20.5C2.67157 19.6716 2.67157 18.3284 3.5 17.5L10.657 10.343C10.919 10.081 11.0499 9.95005 11.1198 9.81949C11.1941 9.68068 11.2176 9.60347 11.2332 9.44681C11.2479 9.29945 11.1999 9.05638 11.1041 8.57024C11.0358 8.22406 11 7.86621 11 7.5C11 4.46243 13.4624 2 16.5 2C17.5055 2 18.448 2.26982 19.259 2.74101ZM12.0001 14.9999L17.5 20.4999C18.3284 21.3283 19.6716 21.3283 20.5 20.4999C21.3284 19.6715 21.3284 18.3283 20.5 17.4999L15.9753 12.9753C15.655 12.945 15.3427 12.8872 15.0408 12.8043C14.6517 12.6975 14.2249 12.7751 13.9397 13.0603L12.0001 14.9999Z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

export const CloudFogIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width={24}
    height={24}
    color={"currentColor"}
    fill={"none"}
    {...props}
  >
    <path
      d="M17.4776 9.00005C17.485 9.00002 17.4925 9 17.5 9C19.9853 9 22 11.0147 22 13.5C22 15.9853 19.9853 18 17.5 18H7C4.23858 18 2 15.7614 2 13C2 10.4003 3.98398 8.26407 6.52042 8.0227M17.4776 9.00005C17.4924 8.83536 17.5 8.66856 17.5 8.5C17.5 5.46243 15.0376 3 12 3C9.12324 3 6.76233 5.20862 6.52042 8.0227M17.4776 9.00005C17.3753 10.1345 16.9286 11.1696 16.2428 12M6.52042 8.0227C6.67826 8.00768 6.83823 8 7 8C8.12582 8 9.16474 8.37209 10.0005 9"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M6 21H8"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
    />
    <path
      d="M11 21H13"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
    />
    <path
      d="M16 21H18"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
    />
  </svg>
);

export const Gmail: React.FC<IconProps> = (props) => {
  return (
    <svg
      fill="transparent"
      height="100"
      viewBox="0 0 256 193"
      width="100"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M58.182 192.05V93.14L27.507 65.077L0 49.504v125.091c0 9.658 7.825 17.455 17.455 17.455z"
        fill="#4285f4"
      />
      <path
        d="M197.818 192.05h40.727c9.659 0 17.455-7.826 17.455-17.455V49.505l-31.156 17.837l-27.026 25.798z"
        fill="#34a853"
      />
      <path
        d="m58.182 93.14l-4.174-38.647l4.174-36.989L128 69.868l69.818-52.364l4.669 34.992l-4.669 40.644L128 145.504z"
        fill="#ea4335"
      />
      <path
        d="M197.818 17.504V93.14L256 49.504V26.231c0-21.585-24.64-33.89-41.89-20.945z"
        fill="#fbbc04"
      />
      <path
        d="m0 49.504l26.759 20.07L58.182 93.14V17.504L41.89 5.286C24.61-7.66 0 4.646 0 26.23z"
        fill="#c5221f"
      />
    </svg>
  );
};

export const BubbleConversationChatIcon: React.FC<IconProps> = (props) => (
  <svg
    color={"#9b9b9b"}
    fill={"none"}
    height={24}
    viewBox="0 0 24 24"
    width={24}
    xmlns="http://www.w3.org/2000/svg"
    style={{ outline: "none", pointerEvents: "none" }}
    {...props}
  >
    <path
      d="M20 9C19.2048 5.01455 15.5128 2 11.0793 2C6.06549 2 2 5.85521 2 10.61C2 12.8946 2.93819 14.9704 4.46855 16.5108C4.80549 16.85 5.03045 17.3134 4.93966 17.7903C4.78982 18.5701 4.45026 19.2975 3.95305 19.9037C5.26123 20.1449 6.62147 19.9277 7.78801 19.3127C8.20039 19.0954 8.40657 18.9867 8.55207 18.9646C8.65392 18.9492 8.78659 18.9636 9 19.0002"
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="1.5"
    />
    <path
      d="M11 16.2617C11 19.1674 13.4628 21.5234 16.5 21.5234C16.8571 21.5238 17.2132 21.4908 17.564 21.425C17.8165 21.3775 17.9428 21.3538 18.0309 21.3673C18.119 21.3807 18.244 21.4472 18.4938 21.58C19.2004 21.9558 20.0244 22.0885 20.8169 21.9411C20.5157 21.5707 20.31 21.1262 20.2192 20.6496C20.1642 20.3582 20.3005 20.075 20.5046 19.8677C21.4317 18.9263 22 17.6578 22 16.2617C22 13.356 19.5372 11 16.5 11C13.4628 11 11 13.356 11 16.2617Z"
      stroke="currentColor"
      strokeLinejoin="round"
      strokeWidth="1.5"
    />
  </svg>
);

export const HeartHandIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    width="100%"
    height="100%"
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M6 20.0872H8.61029C8.95063 20.0872 9.28888 20.1277 9.61881 20.2087L12.3769 20.8789C12.9753 21.0247 13.5988 21.0389 14.2035 20.9214L17.253 20.3281C18.0585 20.1712 18.7996 19.7855 19.3803 19.2205L21.5379 17.1217C22.154 16.5234 22.154 15.5524 21.5379 14.9531C20.9832 14.4135 20.1047 14.3527 19.4771 14.8103L16.9626 16.6449C16.6025 16.9081 16.1643 17.0498 15.7137 17.0498H13.2855L14.8311 17.0498C15.7022 17.0498 16.4079 16.3633 16.4079 15.5159V15.2092C16.4079 14.5055 15.9156 13.892 15.2141 13.7219L12.8286 13.1418C12.4404 13.0476 12.0428 13 11.6431 13C10.6783 13 8.93189 13.7988 8.93189 13.7988L6 15.0249M2 14.6L2 20.4C2 20.9601 2 21.2401 2.10899 21.454C2.20487 21.6422 2.35785 21.7951 2.54601 21.891C2.75992 22 3.03995 22 3.6 22H4.4C4.96005 22 5.24008 22 5.45399 21.891C5.64215 21.7952 5.79513 21.6422 5.89101 21.454C6 21.2401 6 20.9601 6 20.4V14.6C6 14.04 6 13.7599 5.89101 13.546C5.79513 13.3579 5.64215 13.2049 5.45399 13.109C5.24008 13 4.96005 13 4.4 13H3.6C3.03995 13 2.75992 13 2.54601 13.109C2.35785 13.2049 2.20487 13.3579 2.10899 13.546C2 13.7599 2 14.04 2 14.6ZM17.1914 3.59227C16.5946 2.34341 15.2186 1.6818 13.8804 2.32039C12.5423 2.95898 11.9722 4.4734 12.5325 5.80284C12.8787 6.62448 13.8707 8.22002 14.5781 9.31905C14.8394 9.72513 14.9701 9.92817 15.161 10.0469C15.3247 10.1488 15.5297 10.2037 15.7224 10.1974C15.9471 10.1899 16.1618 10.0794 16.5911 9.85845C17.7532 9.26033 19.4101 8.37457 20.1208 7.83614C21.2707 6.96494 21.5556 5.36359 20.6947 4.14626C19.8337 2.92892 18.3327 2.80914 17.1914 3.59227Z"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

export const BookIcon1 = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    fill="none"
    height="100%"
    viewBox="0 0 24 24"
    width="100%"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M12 21L11.8999 20.8499C11.2053 19.808 10.858 19.287 10.3991 18.9098C9.99286 18.5759 9.52476 18.3254 9.02161 18.1726C8.45325 18 7.82711 18 6.57482 18H5.2C4.07989 18 3.51984 18 3.09202 17.782C2.71569 17.5903 2.40973 17.2843 2.21799 16.908C2 16.4802 2 15.9201 2 14.8V6.2C2 5.07989 2 4.51984 2.21799 4.09202C2.40973 3.71569 2.71569 3.40973 3.09202 3.21799C3.51984 3 4.07989 3 5.2 3H5.6C7.84021 3 8.96031 3 9.81596 3.43597C10.5686 3.81947 11.1805 4.43139 11.564 5.18404C12 6.03968 12 7.15979 12 9.4M12 21V9.4M12 21L12.1001 20.8499C12.7947 19.808 13.142 19.287 13.6009 18.9098C14.0071 18.5759 14.4752 18.3254 14.9784 18.1726C15.5467 18 16.1729 18 17.4252 18H18.8C19.9201 18 20.4802 18 20.908 17.782C21.2843 17.5903 21.5903 17.2843 21.782 16.908C22 16.4802 22 15.9201 22 14.8V6.2C22 5.07989 22 4.51984 21.782 4.09202C21.5903 3.71569 21.2843 3.40973 20.908 3.21799C20.4802 3 19.9201 3 18.8 3H18.4C16.1598 3 15.0397 3 14.184 3.43597C13.4314 3.81947 12.8195 4.43139 12.436 5.18404C12 6.03968 12 7.15979 12 9.4"
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    />
  </svg>
);

export const GoogleCalendarIcon: React.FC<IconProps> = (props) => {
  return (
    <svg
      height="100"
      viewBox="0 0 256 256"
      width="100"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path d="M195.368 60.632H60.632v134.736h134.736z" fill="#fff" />
      <path
        d="M195.368 256L256 195.368l-30.316-5.172l-30.316 5.172l-5.533 27.73z"
        fill="#ea4335"
      />
      <path
        d="M0 195.368v40.421C0 246.956 9.044 256 20.21 256h40.422l6.225-30.316l-6.225-30.316l-33.033-5.172z"
        fill="#188038"
      />
      <path
        d="M256 60.632V20.21C256 9.044 246.956 0 235.79 0h-40.422q-5.532 22.554-5.533 33.196q0 10.641 5.533 27.436q20.115 5.76 30.316 5.76T256 60.631"
        fill="#1967d2"
      />
      <path d="M256 60.632h-60.632v134.736H256z" fill="#fbbc04" />
      <path d="M195.368 195.368H60.632V256h134.736z" fill="#34a853" />
      <path
        d="M195.368 0H20.211C9.044 0 0 9.044 0 20.21v175.158h60.632V60.632h134.736z"
        fill="#4285f4"
      />
      <path
        d="M88.27 165.154c-5.036-3.402-8.523-8.37-10.426-14.94l11.689-4.816q1.59 6.063 5.558 9.398c2.627 2.223 5.827 3.318 9.566 3.318q5.734 0 9.852-3.487c2.746-2.324 4.127-5.288 4.127-8.875q0-5.508-4.345-8.994c-2.897-2.324-6.535-3.486-10.88-3.486h-6.754v-11.57h6.063q5.608 0 9.448-3.033c2.56-2.02 3.84-4.783 3.84-8.303c0-3.132-1.145-5.625-3.435-7.494c-2.29-1.87-5.188-2.813-8.708-2.813c-3.436 0-6.164.91-8.185 2.745a16.1 16.1 0 0 0-4.413 6.754l-11.57-4.817c1.532-4.345 4.345-8.185 8.471-11.503s9.398-4.985 15.798-4.985c4.733 0 8.994.91 12.767 2.745c3.772 1.836 6.736 4.379 8.875 7.613c2.14 3.25 3.2 6.888 3.2 10.93c0 4.126-.993 7.613-2.98 10.476s-4.43 5.052-7.327 6.585v.69a22.25 22.25 0 0 1 9.398 7.327c2.442 3.284 3.672 7.208 3.672 11.79c0 4.58-1.163 8.673-3.487 12.26c-2.324 3.588-5.54 6.417-9.617 8.472c-4.092 2.055-8.69 3.1-13.793 3.1c-5.912.016-11.369-1.685-16.405-5.087m71.797-58.005l-12.833 9.28l-6.417-9.734l23.023-16.607h8.825v78.333h-12.598z"
        fill="#4285f4"
      />
    </svg>
  );
};

export const Github: React.FC<IconProps> = (props) => {
  return (
    <svg height="100" viewBox="0 0 438.549 438.549" width="100" {...props}>
      <path
        d="M409.132 114.573c-19.608-33.596-46.205-60.194-79.798-79.8-33.598-19.607-70.277-29.408-110.063-29.408-39.781 0-76.472 9.804-110.063 29.408-33.596 19.605-60.192 46.204-79.8 79.8C9.803 148.168 0 184.854 0 224.63c0 47.78 13.94 90.745 41.827 128.906 27.884 38.164 63.906 64.572 108.063 79.227 5.14.954 8.945.283 11.419-1.996 2.475-2.282 3.711-5.14 3.711-8.562 0-.571-.049-5.708-.144-15.417a2549.81 2549.81 0 01-.144-25.406l-6.567 1.136c-4.187.767-9.469 1.092-15.846 1-6.374-.089-12.991-.757-19.842-1.999-6.854-1.231-13.229-4.086-19.13-8.559-5.898-4.473-10.085-10.328-12.56-17.556l-2.855-6.57c-1.903-4.374-4.899-9.233-8.992-14.559-4.093-5.331-8.232-8.945-12.419-10.848l-1.999-1.431c-1.332-.951-2.568-2.098-3.711-3.429-1.142-1.331-1.997-2.663-2.568-3.997-.572-1.335-.098-2.43 1.427-3.289 1.525-.859 4.281-1.276 8.28-1.276l5.708.853c3.807.763 8.516 3.042 14.133 6.851 5.614 3.806 10.229 8.754 13.846 14.842 4.38 7.806 9.657 13.754 15.846 17.847 6.184 4.093 12.419 6.136 18.699 6.136 6.28 0 11.704-.476 16.274-1.423 4.565-.952 8.848-2.383 12.847-4.285 1.713-12.758 6.377-22.559 13.988-29.41-10.848-1.14-20.601-2.857-29.264-5.14-8.658-2.286-17.605-5.996-26.835-11.14-9.235-5.137-16.896-11.516-22.985-19.126-6.09-7.614-11.088-17.61-14.987-29.979-3.901-12.374-5.852-26.648-5.852-42.826 0-23.035 7.52-42.637 22.557-58.817-7.044-17.318-6.379-36.732 1.997-58.24 5.52-1.715 13.706-.428 24.554 3.853 10.85 4.283 18.794 7.952 23.84 10.994 5.046 3.041 9.089 5.618 12.135 7.708 17.705-4.947 35.976-7.421 54.818-7.421s37.117 2.474 54.823 7.421l10.849-6.849c7.419-4.57 16.18-8.758 26.262-12.565 10.088-3.805 17.802-4.853 23.134-3.138 8.562 21.509 9.325 40.922 2.279 58.24 15.036 16.18 22.559 35.787 22.559 58.817 0 16.178-1.958 30.497-5.853 42.966-3.9 12.471-8.941 22.457-15.125 29.979-6.191 7.521-13.901 13.85-23.131 18.986-9.232 5.14-18.182 8.85-26.84 11.136-8.662 2.286-18.415 4.004-29.263 5.146 9.894 8.562 14.842 22.077 14.842 40.539v60.237c0 3.422 1.19 6.279 3.572 8.562 2.379 2.279 6.136 2.95 11.276 1.995 44.163-14.653 80.185-41.062 108.068-79.226 27.88-38.161 41.825-81.126 41.825-128.906-.01-39.771-9.818-76.454-29.414-110.049z"
        fill="currentColor"
      />
    </svg>
  );
};

export const DiscordIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="16"
    fill="currentColor"
    viewBox="0 0 16 16"
    {...props}
  >
    <path d="M13.545 2.907a13.2 13.2 0 0 0-3.257-1.011.05.05 0 0 0-.052.025c-.141.25-.297.577-.406.833a12.2 12.2 0 0 0-3.658 0 8 8 0 0 0-.412-.833.05.05 0 0 0-.052-.025c-1.125.194-2.22.534-3.257 1.011a.04.04 0 0 0-.021.018C.356 6.024-.213 9.047.066 12.032q.003.022.021.037a13.3 13.3 0 0 0 3.995 2.02.05.05 0 0 0 .056-.019q.463-.63.818-1.329a.05.05 0 0 0-.01-.059l-.018-.011a9 9 0 0 1-1.248-.595.05.05 0 0 1-.02-.066l.015-.019q.127-.095.248-.195a.05.05 0 0 1 .051-.007c2.619 1.196 5.454 1.196 8.041 0a.05.05 0 0 1 .053.007q.121.1.248.195a.05.05 0 0 1-.004.085 8 8 0 0 1-1.249.594.05.05 0 0 0-.03.03.05.05 0 0 0 .003.041c.24.465.515.909.817 1.329a.05.05 0 0 0 .056.019 13.2 13.2 0 0 0 4.001-2.02.05.05 0 0 0 .021-.037c.334-3.451-.559-6.449-2.366-9.106a.03.03 0 0 0-.02-.019m-8.198 7.307c-.789 0-1.438-.724-1.438-1.612s.637-1.613 1.438-1.613c.807 0 1.45.73 1.438 1.613 0 .888-.637 1.612-1.438 1.612m5.316 0c-.788 0-1.438-.724-1.438-1.612s.637-1.613 1.438-1.613c.807 0 1.451.73 1.438 1.613 0 .888-.631 1.612-1.438 1.612" />
  </svg>
);

export const TwitterIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="16"
    fill="currentColor"
    viewBox="0 0 16 16"
    {...props}
  >
    <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334q.002-.211-.006-.422A6.7 6.7 0 0 0 16 3.542a6.7 6.7 0 0 1-1.889.518 3.3 3.3 0 0 0 1.447-1.817 6.5 6.5 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.32 9.32 0 0 1-6.767-3.429 3.29 3.29 0 0 0 1.018 4.382A3.3 3.3 0 0 1 .64 6.575v.045a3.29 3.29 0 0 0 2.632 3.218 3.2 3.2 0 0 1-.865.115 3 3 0 0 1-.614-.057 3.28 3.28 0 0 0 3.067 2.277A6.6 6.6 0 0 1 .78 13.58a6 6 0 0 1-.78-.045A9.34 9.34 0 0 0 5.026 15" />
  </svg>
);

export const WhatsappIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="16"
    fill="currentColor"
    viewBox="0 0 16 16"
    {...props}
  >
    <path d="M13.601 2.326A7.85 7.85 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.9 7.9 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.9 7.9 0 0 0 13.6 2.326zM7.994 14.521a6.6 6.6 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.56 6.56 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592m3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.73.73 0 0 0-.529.247c-.182.198-.691.677-.691 1.654s.71 1.916.81 2.049c.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232" />
  </svg>
);

export const YoutubeIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="16"
    fill="currentColor"
    viewBox="0 0 16 16"
    {...props}
  >
    <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z" />
  </svg>
);

export const LinkedinIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="16"
    height="16"
    fill="currentColor"
    viewBox="0 0 16 16"
    {...props}
  >
    <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z" />
  </svg>
);

export const ArrowUpRight = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    stroke-width="2"
    stroke-linecap="round"
    stroke-linejoin="round"
    {...props}
  >
    <path d="M7 7h10v10" />
    <path d="M7 17 17 7" />
  </svg>
);
