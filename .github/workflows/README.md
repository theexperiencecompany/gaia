...
1. `select-runner` picks the home box when it is online and idle and falls back to `ubuntu-latest` otherwise; compute lanes land on `fromJSON(needs.select-runner.outputs.runner)`. `code-quality.yml` runs the same job for its own lanes.
