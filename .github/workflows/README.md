...
flowchart TD
  ...
  subgraph BACKEND_DEPLOY["deploy-swarm-prod.yml (Swarm app stack)"]
    direction TB
    DEPLOY_BACKEND --> D_AUTH["Tailscale tag:ci join + GHCR login"]:::deploy
    D_AUTH --> D_CTX["Docker context over Tailscale SSH"]:::deploy
...
0. THE pipeline for `pull_request` to `master` and pushes to `master`, plus `workflow_dispatch` (which additionally enables the `probe` job and the `force_home` smoke test). Concurrency is per SHA — group `ci-<ref>-<sha>` with `cancel-in-progress: true`, so a new push never waits on the old run's cancellation; superseded runs of the branch are cancelled through the API by `scripts/ci/runner.sh cancel-superseded` in `select-runner`.
1. `select-runner` picks the home box when it is online (queueing behind a busy pool — its setup is ~30x cheaper than a GitHub runner's) and falls back to `ubuntu-latest` when it is offline/unreachable, for every fork PR, and when the `force_github` dispatch input is set; compute lanes land on `fromJSON(needs.select-runner.outputs.runner)`. `runner-watchdog` (GitHub-hosted, not in the gate) cancels a run whose self-hosted jobs stay queued past 8 minutes, so a box that dies after selection never parks a PR for GitHub's 24 h queue limit. `code-quality.yml` runs the same selection for its own lanes.
...
3. Correctness lanes, each gated on the affected lists: `build` (TS builds), `test-typescript` (vitest via Nx), and `test-python` — a four-slice matrix (`unit-a` = `tests/unit/{services,agents,storage}` and `unit-b` = the rest of `tests/unit` + `tests/meta`, both without services/Node; `integration` = the real/e2e/stress/integration/contracts trees with services + Node; `bridge` = `tests/integration/real/test_device_bridge_e2e.py` alone, serial, because it TRUNCATEs `bridge_devices` on the lane's shared Postgres). Tests run directly on the runner against live PostgreSQL/Redis/MongoDB/ChromaDB/RabbitMQ containers (`scripts/ci/test-services.sh`, or the shared namespaces via `scripts/ci/test-services.sh` on the box). Coverage is traced only on master/dispatch runs (PR lanes set no `COVERAGE`); each slice then writes its own `.coverage.<slice>` with `--cov-fail-under=0` and the separate `coverage` job downloads all four, `coverage combine`s them and enforces the threshold — `fail_under` 80 (`apps/api/pyproject.toml`). The job itself does not run on PRs: PR lanes trace no coverage, and a test-impact selection would make 80 unreachable by construction. Static checks (ruff, mypy, Biome, tsc, custom AST lints, dead code) intentionally do NOT run here — they are enforced lanes in `code-quality.yml`.
...
1. Enter from every PR regardless of its base branch (a PR stacked on another PR's branch gets the same gate; `main.yml` does the same, and its `regression-proof` lane diffs against `origin/<base_ref>`), pushes to `master`, and manual dispatch.
...
2. `docker-release`: detect affected backend/bot projects, publish images to GHCR, optionally sync Discord commands. Records `images_published` right after its push steps (before Discord command sync), so a later, unrelated step failure never misreports a real publish as "nothing published". After the release steps, `scripts/ci/release.sh resolve-image-tags` resolves the immutable per-commit tags nx pushed (production versionScheme `YYMM.DD.<shortsha>`, read from the local image store, never recomputed) and guarantees they exist in GHCR (pushing them if nx skipped publishing), emitting `apps_tag` (gaia + gaia-voice-agent) and `bots_tag` (the five bot images) — empty when that group wasn't released this run. Backend `:latest` is never pushed at build time: it is re-pointed exclusively by the deploy's ``deploy.sh retag`` after convergence (":latest == deployed").
...
5. `deployment-plan` runs with `if: always()` — it is never skipped by a lane failing or being cancelled — and needs `docker-release` + `docker-grafana` purely for sequencing (deploy planning must not race ahead of the build lanes). It evaluates `docker-release`'s `.result` plus backend affected-detection (`docker-release.outputs.api_affected`/`bots_affected`) via `scripts/ci/deploy.sh plan`:
...
6. `notify-publish-without-deploy` fires a Discord alert when `backend_orphaned` is set, naming the published-but-undeployed images and the `gh workflow run` remedy above. All deploy-pipeline Discord sends go through `scripts/ci/deploy.sh notify` (single webhook embed; the previous appleboy action 400'd on messages over 256 chars and fragmented comma-containing ones), with `continue-on-error` on the send step only — the message is fully logged in the run either way, and a dropped webhook must not turn an otherwise-green run red.
...
3. Only after convergence succeeds, `scripts/ci/deploy.sh retag` re-points each deployed repo's `:latest` at the deployed tag registry-side (`docker buildx imagetools create`, no layer transfer) — `:latest` is a deploy-time alias meaning "what prod runs", not "what was last built". A failed deploy does not move `:latest`.
4. Push a deploy annotation to Loki (including the deployed tags) and send status to Discord via `scripts/ci/deploy.sh notify`.
5. The runner reaches the VM over Tailscale SSH (`setup-swarm-context` joins as an ephemeral `tag:ci` node and logs in by tailnet identity — no SSH key secret; the Hetzner firewall has no public SSH rule). Manual `workflow_dispatch` supports `action=verify`: same private checkout and prod context, but `docker stack config` renders the stack instead of deploying it — run it after touching the infra repo, the deploy key, or the tailnet path. It also supports `action=rollback` with `rollback_mode=last` (Docker service rollback) or `rollback_mode=digest` (redeploy pinned image). After a successful rollback, the same retag script re-points `:latest` at the images the rolled-back services now run (digest mode: the pinned gaia ref; last mode: each rolled-back service's restored image spec), so the alias keeps tracking production through rollbacks too.
...
4. `preview`: gated on a pull request whose head branch lives in this repo and whose actor is not `dependabot[bot]` — Actions secrets are not exposed to Dependabot or fork PRs, so `CLOUDFLARE_API_TOKEN` would arrive empty and `wrangler` would exit 1 on a step that could never have succeeded. A skipped `preview` on such a PR is expected, not a regression; `build` still runs there, so the bundle is always validated. Uploads preview version via `command: versions upload --preview-alias pr-${{ github.event.number }} --config apps/web/wrangler.jsonc`, reports duration, comments PR with expected preview URL. Requires Workers Versions / Gradual Rollouts enabled; `preview_urls: false` in `wrangler.jsonc` means the alias URL is only reachable if routing is configured.

> Workers Builds (dashboard git auto-deploy) must be **disabled** — see `docs/cloudflare-workers-builds.md` and `scripts/ci/release.sh disable-cf-builds`. The workflow header comments the exact dashboard steps and the `preflight` API probe.
...
- `.github/workflows/deploy-web.yml`: Cloudflare Workers frontend deploy — builds `pnpm --filter ./apps/web cf:build`, deploys via `cloudflare/wrangler-action@v3` on `push: master` (paths `apps/web/**`), preview alias `pr-<n>` on PRs from branches in this repo (Dependabot and fork PRs skip `preview` — no Actions secrets). Reports duration, fails visibly, uses `environment: production`. Workers Builds auto-deploy must be disabled (see `docs/cloudflare-workers-builds.md`).
