...
3. Fifteen hygiene lanes (Biome, deps, circular, file-size, types-location, components-per-file, jscpd, package hygiene, tsc, ruff + custom AST lints, mypy, interrogate, xenon, bandit, knip/vulture dead code), each self-scoping to changed files via `scripts/ci/changed-files.sh`.
