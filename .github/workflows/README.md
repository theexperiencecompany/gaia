...
3. Correctness lanes, each gated on the affected lists: `build` (TS builds), `test-typescript` (vitest via Nx), and `test-python` — a four-slice matrix (`unit-a` = `tests/unit/{services,agents,storage}` and `unit-b` = the rest of `tests/unit` + `tests/meta`, both without services/Node; `integration` = the real/e2e/stress/integration/contracts trees with services + Node; `bridge` = `tests/integration/real/test_device_bridge_e2e.py` alone, serial, because it TRUNCATEs `bridge_devices` on the lane's shared Postgres). Tests run directly on the runner against live PostgreSQL/Redis/MongoDB/ChromaDB/RabbitMQ containers (`scripts/ci/start-test-services.sh`, or the shared namespaces via `scripts/ci/shared-test-services.sh` on the box). Coverage is traced only on master/dispatch runs (PR lanes set no `COVERAGE`); each slice then writes its own `.coverage.<slice>` with `--cov-fail-under=0` and the separate `coverage` job downloads all four, `coverage combine`s them and enforces the threshold — `fail_under` 80 (`apps/api/pyproject.toml`). The job itself does not run on PRs: PR lanes trace no coverage, and a test-impact selection would make 80 unreachable by construction. Static checks (ruff, mypy, Biome, tsc, custom AST lints, dead code) intentionally do NOT run here — they are enforced lanes in `code-quality.yml`.
