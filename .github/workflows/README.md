...
3. Seventeen hygiene lanes (Biome, deps, circular, file-size, types-location, components-per-file, jscpd, type-coverage, package hygiene, tsc, ruff + custom AST lints, mypy, interrogate, xenon, bandit, evlog-map observability score, knip/vulture dead code), each self-scoping to changed files via `scripts/ci/changed-files.sh`. The observability lane (`tools/evlog_map`, currently informational) posts the full-repo score to the job summary and fails PRs whose changed files score below the same files at the merge-base.
