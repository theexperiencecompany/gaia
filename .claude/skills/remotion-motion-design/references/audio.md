...
- **VO cues must not overlap — verify arithmetically.** For every consecutive pair, check `cue.at + cue.durationInFrames < next.at` (durations from the real trimmed files, not estimates). Two voices overlapping for even 10 frames is an instant illusion-breaker. When the script is too dense for the timeline, CUT a line rather than compress — on-screen text already carries any line that duplicates it.
