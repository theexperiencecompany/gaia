...
4. **Morphing is the seam language.** Prefer transformation over replacement everywhere: cards compress into chips that fly into the next surface, a giant element becomes a small persistent one, digits roll in place. Every seam should answer "what did the last thing BECOME?"
   - **Odometer limit:** per-digit rolling reads beautifully when 1–2 digits change; when ≥3 change at once (5:41→7:00) the mid-roll frames show two stacked half-times — garbage. Detect the changed-digit count and fall back to a whole-string blur-flash swap (blur up → swap → blur settle) for big jumps.
