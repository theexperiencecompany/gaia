...
- Self-wiring flow: when an outbound action succeeds inside a todo-bound run — email sent, draft sent, thread replied to, calendar event created — the tool result is appended with an instruction to subscribe the active todo, naming the `thread_id`/`event_id` the response returned. The model then calls the ordinary subscription tool, so there is one creation path and the subscription is a visible tool call rather than a side effect. The append runs in the agent middleware tool-call wrapper, the only interception point that knows the run's active todo.
...
- New `awrap_tool_call` middleware in `app/agents/middleware/` registered in `create_middleware_stack` for the self-wiring capture
