...
- [x] 5.5 Add `list_available_triggers`: the intersection of the matchable catalog with the user's *connected* integrations (reuses `TriggerService.get_all_workflow_triggers` + `get_connected_integration_ids`), so the model sees real slugs for this user instead of learning them via the wrong-name error path — and never subscribes to an integration that cannot fire. Registered in `build_graph.py` and both prompt tool lists; unit-tested on the connected-filter (linear is catalogued but, when unconnected, must not be offered)
